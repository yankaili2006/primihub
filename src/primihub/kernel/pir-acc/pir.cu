...
void verify_encryption_params(const sigma::EncryptionParameters &enc_params) {
  SIGMAContext context(enc_params, true);
...
uint64_t invert_mod(uint64_t m, const sigma::Modulus &mod) {
  ...
  uint64_t inverse = 0;
  if (!sigma::util::try_invert_uint_mod(m, mod.value(), inverse)) {
...
__global__ void g_decmpose_to_plaintexts(uint64_t* operand, uint64_t* ct, size_t shift, uint64_t pt_bitmask){
    auto tid = blockDim.x * blockIdx.x + threadIdx.x;
    operand[tid] = (ct[tid] >> shift) & pt_bitmask;
}
  ...
  for (size_t poly_index = 0; poly_index < ct.size(); ++poly_index) {
    for (size_t coeff_mod_index = 0; coeff_mod_index < coeff_mod_count; ++coeff_mod_index) {
      ...
      for (size_t i = 0; i < local_expansion_ratio; ++i) {
        Plaintext temp;
        temp.device_resize(coeff_count);
        //pt_iter->resize(coeff_count);
        int blocknum = ceil(coeff_count / 1024.0);
          g_decmpose_to_plaintexts<<<blocknum, 1024>>>(temp.device_data(), (uint64_t*)ct.device_data() +
          poly_index * coeff_count * coeff_mod_count + coeff_mod_index * coeff_count, shift, pt_bitmask);
        /*for (size_t c = 0; c < coeff_count; ++c) {
          (*pt_iter)[c] = (ct.data(poly_index)[coeff_mod_index * coeff_count + c] >> shift) & pt_bitmask;
        }*/
        pt_iter->device_resize(coeff_count);
        KernelProvider::retrieve(pt_iter->device_data(), temp.device_data(), coeff_count);

        //pt_iter->copy_to_device();
        ++pt_iter;
...
PirQuery deserialize_query(uint32_t d, uint32_t count, string s,
                           ...
                           shared_ptr<SIGMAContext> context) {
...
GaloisKeys *deserialize_galoiskeys(string s, shared_ptr<SIGMAContext> context) {
