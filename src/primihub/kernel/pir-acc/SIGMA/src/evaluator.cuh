...
namespace sigma
{
    ...
    */
    ...
    public:
        /**
        Creates an Evaluator instance initialized with the specified SIGMAContext.

        @param[in] context The SIGMAContext
        @throws std::invalid_argument if the encryption parameters are not valid
        */
        Evaluator(const SIGMAContext &context);
        ...
        */
        ...
        void cu_add_inplace(Ciphertext &encrypted1, Ciphertext &encrypted2) const;
        /**
        ...
        */
        ...
        void cu_multiply_plain(Ciphertext &encrypted, Plaintext &plain, Ciphertext &destination);
        ...
        */
            ...
            Ciphertext &destination, MemoryPoolHandle pool = MemoryManager::GetPool()) const
        {
            ...
            destination.copy_device_from_host(encrypted);
            sealpir_apply_galois_inplace(destination, galois_elt, galois_keys, std::move(pool));
        }
        ...
        */
        ...
        void preprocess_transform_to_ntt_inplace(Plaintext &plain, parms_id_type parms_id);

        void sealpir_transform_to_ntt_inplace(Plaintext &plain, parms_id_type parms_id);

        void sealpir_transform_to_ntt_inplace(Ciphertext &encrypted);

        void sealpir_transform_from_ntt_inplace(Ciphertext &encrypted_ntt);

        void sealpir_apply_galois_inplace(
                Ciphertext &encrypted, uint32_t galois_elt,
                const GaloisKeys &galois_keys, MemoryPoolHandle pool = MemoryManager::GetPool()) const;

        void sealpir_switch_key_inplace(
                Ciphertext &encrypted, uint64_t* target_iter, const KSwitchKeys &kswitch_keys, size_t kswitch_keys_index,
                MemoryPoolHandle pool) const;

        void cu_add(Ciphertext &encrypted1, const Ciphertext &encrypted2, Ciphertext &result);

        void cu_multiply_plain_ntt(Ciphertext &encrypted_ntt, Plaintext &plain_ntt);

        void cu_multiply_plain_normal(Ciphertext &encrypted, Plaintext &plain) ;

        void cu_mod_switch_to_inplace(Ciphertext &encrypted, parms_id_type parms_id);
...
/*        void negacyclic_multiply_poly_mono_coeffmod(
                ...
                util::ConstModulusIter modulus, uint64_t* result);*/

        /**
        Enables access to private members of sigma::Evaluator for SIGMA_C.
        */
    ...
    private:
        ...
        mutable util::DeviceArray<uint64_t> temp_;
