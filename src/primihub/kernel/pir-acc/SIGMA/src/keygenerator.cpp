...
namespace sigma
{
    KeyGenerator::KeyGenerator(const SIGMAContext &context) : context_(context)
    {
        // Verify parameters
    ...
    KeyGenerator::KeyGenerator(const SIGMAContext &context, const SecretKey &secret_key) : context_(context)
    {
        // Verify parameters
        if (!context_.parameters_set())
        {
    ...
    RelinKeys KeyGenerator::create_relin_keys(size_t count, bool save_seed)
    {
        ...
        if (!count || count > SIGMA_CIPHERTEXT_SIZE_MAX - 2)
        {
    ...
    GaloisKeys KeyGenerator::create_galois_keys(const vector<uint32_t> &galois_elts, bool save_seed)
    {
        ...
        for (auto galois_elt : galois_elts)
        {
            ...
            SIGMA_ALLOCATE_GET_RNS_ITER(rotated_secret_key, coeff_count, coeff_modulus_size, pool_);
            RNSIter secret_key(secret_key_.data().data(), coeff_count);
    ...
    void KeyGenerator::compute_secret_key_array(const SIGMAContext::ContextData &context_data, size_t max_power)
    {
#ifdef SIGMA_DEBUG
        if (max_power < 1)
        {
        ...
        SIGMA_ITERATE(iter(secret_key_power, next_secret_key_power), new_size - old_size, [&](auto I) {
    ...
    void KeyGenerator::generate_one_kswitch_key(ConstRNSIter new_key, vector<PublicKey> &destination, bool save_seed)
    {
        ...
        //s
        SIGMA_ITERATE(iter(new_key, key_modulus, destination, size_t(0)), decomp_mod_count, [&](auto I) {
            SIGMA_ALLOCATE_GET_COEFF_ITER(temp, coeff_count, pool_);
            encrypt_zero_symmetric(//(-as+e),a
        ...
        ConstPolyIter new_keys, size_t num_keys, KSwitchKeys &destination, bool save_seed)
    {
        ...
        SIGMA_ITERATE(iter(new_keys, destination.data()), num_keys, [&](auto I) {
