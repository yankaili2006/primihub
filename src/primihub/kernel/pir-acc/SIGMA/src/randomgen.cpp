...
namespace sigma
{
    void random_bytes(sigma_byte *buf, size_t count)
    {
#if SIGMA_SYSTEM == SIGMA_SYSTEM_UNIX_LIKE
...
#elif SIGMA_SYSTEM == SIGMA_SYSTEM_WINDOWS
...
#elif SIGMA_SYSTEM == SIGMA_SYSTEM_OTHER
#warning "SECURITY WARNING: System detection failed; falling back to a potentially insecure randomness source!"
    ...
    void UniformRandomGeneratorInfo::load_members(istream &stream, SIGMA_MAYBE_UNUSED SIGMAVersion version)
    {
        // Throw exceptions on std::ios_base::badbit and std::ios_base::failbit
        auto old_except_mask = stream.exceptions();
        try
    ...
    void UniformRandomGenerator::generate(size_t byte_count, sigma_byte *destination)
    {
    ...
    {
        static shared_ptr<UniformRandomGeneratorFactory> default_factory{ new SIGMA_DEFAULT_PRNG_FACTORY() };
    ...
    void Shake256PRNG::refill_buffer()
    {
        ...
        sigma_memzero(seed_ext.data(), seed_ext.size() * bytes_per_uint64);
        counter_++;
    }
} // namespace sigma
