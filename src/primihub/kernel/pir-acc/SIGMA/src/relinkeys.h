...
namespace sigma
{
    ...
    the size of ciphertexts back to 2. Microsoft SIGMA can only relinearize size 3
    ...
    */
    ...
    public:
        ...
        */
        SIGMA_NODISCARD inline static std::size_t get_index(std::size_t key_power)
        {
            if (key_power < 2)
            {
        ...
        @throws std::invalid_argument if key_power is less than 2
        */
        SIGMA_NODISCARD inline bool has_key(std::size_t key_power) const
        {
        ...
        */
        SIGMA_NODISCARD inline auto &key(std::size_t key_power) const
        {
