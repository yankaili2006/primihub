...
namespace sigma
{
    ...
    void EncryptionParameters::load_members(istream &stream, SIGMA_MAYBE_UNUSED SIGMAVersion version)
    {
        // Throw exceptions on std::ios_base::badbit and std::ios_base::failbit
        auto old_except_mask = stream.exceptions();
        try
        {
            ...
            if (poly_modulus_degree64 > SIGMA_POLY_MOD_DEGREE_MAX)
            {
            ...
            // Only check for upper bound; lower bound is zero for scheme_type::none
            if (coeff_modulus_size64 > SIGMA_COEFF_MOD_COUNT_MAX)
            {
