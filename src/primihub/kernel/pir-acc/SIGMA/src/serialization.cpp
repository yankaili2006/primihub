...
namespace sigma
{
    ...
    // Required for C++14 compliance: static constexpr member variables are not necessarily inlined so need to ensure
    // symbol is created.
    constexpr uint16_t Serialization::sigma_magic;

    // Required for C++14 compliance: static constexpr member variables are not necessarily inlined so need to ensure
    // symbol is created.
    constexpr uint8_t Serialization::sigma_header_size;
    ...
    streamoff Serialization::SaveHeader(const SIGMAHeader &header, ostream &stream)
    {
        ...
        try
        {
            ...
            stream.write(reinterpret_cast<const char *>(&header), sizeof(SIGMAHeader));
        }
        ...
        // Return the size of the SIGMAHeader
        return static_cast<streamoff>(sizeof(SIGMAHeader));
    }

    streamoff Serialization::LoadHeader(istream &stream, SIGMAHeader &header, bool try_upgrade_if_invalid)
    {
        auto old_except_mask = stream.exceptions();
        try
        {
            ...
            stream.read(reinterpret_cast<char *>(&header), sizeof(SIGMAHeader));
            ...
            {
                // Try interpret the data as a Microsoft SIGMA 3.4 header
                legacy_headers::SIGMAHeader_3_4 header_3_4(header);

                SIGMAHeader new_header;
    ...
    streamoff Serialization::SaveHeader(const SIGMAHeader &header, sigma_byte *out, size_t size)
    {
        if (!out)
        {
            throw invalid_argument("out cannot be null");
        }
        if (size < sizeof(SIGMAHeader))
        {
    ...
    streamoff Serialization::LoadHeader(
        const sigma_byte *in, size_t size, SIGMAHeader &header, bool try_upgrade_if_invalid)
    {
        if (!in)
        {
    ...
    streamoff Serialization::Save(
        ...
        SIGMA_MAYBE_UNUSED bool clear_buffers)
    {
        if (!save_members)
        {
            throw invalid_argument("save_members is invalid");
        }
        if (raw_size < static_cast<streamoff>(sizeof(SIGMAHeader)))
        {
        ...
        try
        {
            ...
            case compr_mode_type::zlib:
            {
                ...
                SafeByteBuffer safe_buffer(
                    ztools::zlib_deflate_size_bound(raw_size - static_cast<streamoff>(sizeof(SIGMAHeader))),
                    clear_buffers);
                ...
                DynArray<sigma_byte> safe_buffer_array(
                    Pointer<sigma_byte>::Aliasing(safe_buffer.data()), safe_buffer.size(),
            ...
            case compr_mode_type::zstd:
            {
                ...
                // the start to avoid extra reallocs.
                SafeByteBuffer safe_buffer(
                    ztools::zstd_deflate_size_bound(raw_size - static_cast<streamoff>(sizeof(SIGMAHeader))),
                    clear_buffers);
                iostream temp_stream(&safe_buffer);
                temp_stream.exceptions(ios_base::badbit | ios_base::failbit);
                save_members(temp_stream);
    ...
    streamoff Serialization::Load(
        function<void(istream &, SIGMAVersion)> load_members, istream &stream, SIGMA_MAYBE_UNUSED bool clear_buffers)
    {
        if (!load_members)
        {
        ...
        try
        {
            ...
            {
                throw logic_error("loaded SIGMAHeader is invalid");
            }

            // Read header version information so we can call, if necessary, the
            // correct variant of load_members.
            SIGMAVersion version{ header.version_major, header.version_minor, 0, 0 };
    ...
    streamoff Serialization::Save(
        function<void(ostream &)> save_members, streamoff raw_size, sigma_byte *out, size_t size,
    ...
    streamoff Serialization::Load(
        function<void(istream &, SIGMAVersion)> load_members, const sigma_byte *in, size_t size, bool clear_buffers)
    {
        if (!in)
        {
            throw invalid_argument("in cannot be null");
