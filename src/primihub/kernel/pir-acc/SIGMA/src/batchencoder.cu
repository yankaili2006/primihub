...
namespace sigma
{
    BatchEncoder::BatchEncoder(const SIGMAContext &context) : context_(context)
    {
        // Verify parameters
    ...
    void BatchEncoder::populate_roots_of_unity_vector(const SIGMAContext::ContextData &context_data)
    {
    ...
    void BatchEncoder::encode(const vector<uint64_t> &values_matrix, Plaintext &destination) const
    {
        ...
        size_t values_matrix_size = values_matrix.size();//4096
        //printf("%d ",values_matrix_size - slots_);
        if (values_matrix_size > slots_)//4096
        {
            throw invalid_argument("values_matrix size is too large");
        }
#ifdef SIGMA_DEBUG
        uint64_t modulus = context_data.parms().plain_modulus().value();//检查系数有没有大于模的
...
/*        uint64_t* temp = (uint64_t*)malloc(4096*sizeof(uint64_t));
        std::copy(destination.data(),destination.data()+4096,temp);*/
       ...
       // kernel_util::g_inv_ntt_negacyclic_harvey(destination.data(), 4096, *context_data.plain_ntt_tables());
        /*for(int i=0;i<4096;i++){
            cout<<temp[i]<<" ";
        }*/
        //cout<<endl<<endl;
        uint64_t* d_temp=KernelProvider::malloc<uint64_t>(4096);
        KernelProvider::copy(d_temp, destination.data(), 4096);
        //sigma::kernel_util::g_inv_ntt_negacyclic_harvey(d_temp, 4096 ,*context_data.device_plain_ntt_tables());
        sigma::kernel_util::d_inverse_ntt_negacyclic_harvey(d_temp,1,1,12,*context_data.device_plain_ntt_tables());
        KernelProvider::retrieve(destination.data(), d_temp, 4096);
    ...
    void BatchEncoder::decode(const Plaintext &plain, vector<uint64_t> &destination, MemoryPoolHandle pool) const
    {
        ...
        ntt_negacyclic_harvey(temp_dest.get(), *context_data.device_plain_ntt_tables());
