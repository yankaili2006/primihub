...
namespace sigma
{
    bool is_metadata_valid_for(const Plaintext &in, const SIGMAContext &context, bool allow_pure_key_levels)
    {
        // Verify parameters
    ...
    bool is_metadata_valid_for(const Ciphertext &in, const SIGMAContext &context, bool allow_pure_key_levels)
    {
        // Verify parameters
        if (!context.parameters_set())
        {
    ...
    bool is_metadata_valid_for(const SecretKey &in, const SIGMAContext &context)
    {
    ...
    bool is_metadata_valid_for(const PublicKey &in, const SIGMAContext &context)
    {
        ...
        // size is minimal (i.e., SIGMA_CIPHERTEXT_SIZE_MIN).
        auto key_parms_id = context.key_parms_id();
        return is_metadata_valid_for(in.data(), context, true) && in.data().is_ntt_form() &&
               (in.parms_id() == key_parms_id) && (in.data().size() == SIGMA_CIPHERTEXT_SIZE_MIN);
    }

    bool is_metadata_valid_for(const KSwitchKeys &in, const SIGMAContext &context)
    {
        // Verify parameters
        if (!context.parameters_set())
        {
    ...
    bool is_metadata_valid_for(const RelinKeys &in, const SIGMAContext &context)
    {
        // Check that the size is within bounds.
        bool size_check =
            !in.size() || (in.size() <= SIGMA_CIPHERTEXT_SIZE_MAX - 2 && in.size() >= SIGMA_CIPHERTEXT_SIZE_MIN - 2);
        return is_metadata_valid_for(static_cast<const KSwitchKeys &>(in), context) && size_check;
    }

    bool is_metadata_valid_for(const GaloisKeys &in, const SIGMAContext &context)
    {
    ...
    bool is_data_valid_for(const Plaintext &in, const SIGMAContext &context)
    {
        // Check metadata
    ...
    bool is_data_valid_for(const Ciphertext &in, const SIGMAContext &context)
    {
        // Check metadata
        if (!is_metadata_valid_for(in, context))
        {
    ...
    bool is_data_valid_for(const SecretKey &in, const SIGMAContext &context)
    {
        // Check metadata
        if (!is_metadata_valid_for(in, context))
        {
    ...
    bool is_data_valid_for(const PublicKey &in, const SIGMAContext &context)
    {
        // Check metadata
        if (!is_metadata_valid_for(in, context))
        {
    ...
    bool is_data_valid_for(const KSwitchKeys &in, const SIGMAContext &context)
    {
        // Verify parameters
        if (!context.parameters_set())
        {
    ...
    bool is_data_valid_for(const RelinKeys &in, const SIGMAContext &context)
    {
        return is_data_valid_for(static_cast<const KSwitchKeys &>(in), context);
    }

    bool is_data_valid_for(const GaloisKeys &in, const SIGMAContext &context)
    {
        return is_data_valid_for(static_cast<const KSwitchKeys &>(in), context);
    }
} // namespace sigma
