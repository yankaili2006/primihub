...
namespace sigma
{
    namespace
    {
            ...
            MemoryPool &pool)
        {
            ...
            SIGMA_ITERATE(poly, coeff_count, [&](auto I) {
    ...
    Decryptor::Decryptor(const SIGMAContext &context, const SecretKey &secret_key) : context_(context)
    {
        // Verify parameters
    ...
    void Decryptor::bfv_decrypt(const Ciphertext &encrypted, Plaintext &destination, MemoryPoolHandle pool)
    {
        ...
        SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(tmp_dest_modq, coeff_count, coeff_modulus_size, pool);
    ...
    void Decryptor::dot_product_ct_sk_array(const Ciphertext &encrypted, RNSIter destination, MemoryPoolHandle pool)
    {
        ...
        else
        {
            ...
            // Now do the dot product of encrypted_copy and the secret key array using NTT.
            // The secret key powers are already NTT transformed.
            SIGMA_ALLOCATE_GET_POLY_ITER(encrypted_copy, encrypted_size - 1, coeff_count, coeff_modulus_size, pool);
            ...
            SIGMA_ITERATE(iter(encrypted_copy, secret_key_array), encrypted_size - 1, [&](auto I) {
            ...
            SIGMA_ITERATE(encrypted_copy, encrypted_size - 1, [&](auto I) {
    ...
    int Decryptor::invariant_noise_budget(const Ciphertext &encrypted)
    {
        ...
        SIGMA_ALLOCATE_ZERO_GET_RNS_ITER(noise_poly, coeff_count, coeff_modulus_size, pool_);
