        ...
        namespace ztools
        {
            namespace
            {
                ...
                public:
                    ...
                    {
                        auto ptr = util::allocate<sigma_byte>(size, pool_);
                ...
                private:
                    ...
                    unordered_map<void *, Pointer<sigma_byte>> ptr_storage_;
                };
            } // namespace
        } // namespace ztools
    } // namespace util
    ...
    namespace util
    {
        namespace ztools
        {
            ...
            int zlib_deflate_array_inplace(DynArray<sigma_byte> &in, MemoryPoolHandle pool)
            {
                ...
                auto temp_out = DynArray<sigma_byte>(buffer_size, pool);

                // Where we are writing output now; start writing to the temporary buffer
                sigma_byte *out_head = temp_out.begin();
                ...
                zstream.next_in = reinterpret_cast<unsigned char *>(const_cast<sigma_byte *>(in.cbegin()));
                ...
                do
                {
                    ...
                    do
                    {
                        ...
                        process_bytes_out =
                            static_cast<size_t>(reinterpret_cast<sigma_byte *>(zstream.next_out) - out_head);
            ...
            void zlib_write_header_deflate_buffer(
                DynArray<sigma_byte> &in, void *header_ptr, ostream &out_stream, MemoryPoolHandle pool)
            {
                Serialization::SIGMAHeader &header = *reinterpret_cast<Serialization::SIGMAHeader *>(header_ptr);
                ...
                header.size = static_cast<uint64_t>(add_safe(sizeof(Serialization::SIGMAHeader), in.size()));
                ...
                try
                {
                    ...
                    out_stream.write(reinterpret_cast<const char *>(&header), sizeof(Serialization::SIGMAHeader));
