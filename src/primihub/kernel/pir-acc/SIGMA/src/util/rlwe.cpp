...
namespace sigma
{
    namespace util
    {
            ...
            shared_ptr<UniformRandomGenerator> prng, const EncryptionParameters &parms, uint64_t *destination)
        {
            ...
            SIGMA_ITERATE(iter(destination), coeff_count, [&](auto &I) {
            ...
            shared_ptr<UniformRandomGenerator> prng, const EncryptionParameters &parms, uint64_t *destination)
        {
            ...
            auto cbd = [&]() {
                unsigned char x[6];
                prng->generate(6, reinterpret_cast<sigma_byte *>(x));
                x[2] &= 0x1F;
                x[5] &= 0x1F;
            ...
            shared_ptr<UniformRandomGenerator> prng, const EncryptionParameters &parms, uint64_t *destination)
        {
            ...
            prng->generate(dest_byte_count, reinterpret_cast<sigma_byte *>(destination));
            ...
            for (size_t j = 0; j < coeff_modulus_size; j++)
            {
                ...
                transform(destination, destination + coeff_count, destination, [&](uint64_t rand) {
                    ...
                    {
                        prng->generate(sizeof(uint64_t), reinterpret_cast<sigma_byte *>(&rand));
                    }
            ...
            Ciphertext &destination)
        {
            ...
            for (size_t j = 0; j < encrypted_size; j++)
            {
                SIGMA_NOISE_SAMPLER(prng, parms, u.get());
            ...
            bool save_seed, Ciphertext &destination)
        {
            ...
            for (size_t i = 0; i < coeff_modulus_size; i++)
            {
                ...
                {
                    inverse_ntt_negacyclic_harvey(c0 + i * coeff_count, h_ntt_tables[i]);
                }
            ...
            if (!is_ntt_form && !save_seed)
            {
                for (size_t i = 0; i < coeff_modulus_size; i++)
                {
                    ...
                    inverse_ntt_negacyclic_harvey(c1 + i * coeff_count, h_ntt_tables[i]);
                }
            }
            ...
            if (save_seed)
            {
                ...
                prng_info.save(reinterpret_cast<sigma_byte *>(c1 + 1), prng_info_byte_count, compr_mode_type::none);
            }
        }
    } // namespace util
} // namespace sigma
