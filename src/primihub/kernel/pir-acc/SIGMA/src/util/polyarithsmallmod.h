...
namespace sigma
{
    namespace util
    {
            ...
            ConstRNSIter poly, std::size_t coeff_modulus_size, ConstModulusIter modulus, RNSIter result)
        {
            ...
            SIGMA_ITERATE(iter(poly, modulus, result), coeff_modulus_size, [&](auto I) {
            ...
            ConstPolyIter poly_array, std::size_t size, ConstModulusIter modulus, PolyIter result)
        {
            ...
            SIGMA_ITERATE(iter(poly_array, result), size, [&](auto I) {
            ...
            ConstCoeffIter poly, std::size_t coeff_count, const Modulus &modulus, CoeffIter result)
        {
            ...
            SIGMA_ITERATE(iter(poly, result), coeff_count, [&](auto I) {
                auto coeff = get<0>(I);
#ifdef SIGMA_DEBUG
...
#endif
            auto poly_modulus_degree = result.poly_modulus_degree();
            SIGMA_ITERATE(iter(operand1, operand2, modulus, result), coeff_modulus_size, [&](auto I) {
...
#endif
            auto coeff_modulus_size = result.coeff_modulus_size();
            SIGMA_ITERATE(iter(operand1, operand2, result), size, [&](auto I) {
...
#endif
            auto poly_modulus_degree = result.poly_modulus_degree();
            SIGMA_ITERATE(iter(poly, mono_coeff, modulus, result), coeff_modulus_size, [&](auto I) {
                negacyclic_multiply_poly_mono_coeffmod(
