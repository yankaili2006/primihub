...
namespace sigma
{
    namespace util
    {
            ...
            CoeffIter result)
        {
            ...
            SIGMA_ITERATE(iter(operand1, operand2, result), coeff_count, [&](auto I) {
#ifdef SIGMA_DEBUG
                ...
                get<2>(I) = SIGMA_COND_SELECT(sum >= modulus_value, sum - modulus_value, sum);
            });
#endif
        }
            ...
            CoeffIter result)
        {
            ...
            SIGMA_ITERATE(iter(operand1, operand2, result), coeff_count, [&](auto I) {
                ...
                get<2>(I) = SIGMA_COND_SELECT(tmp3 >= modulus_value, tmp3 - modulus_value, tmp3);
            });
#endif
        }
        ...
        uint64_t poly_infty_norm_coeffmod(ConstCoeffIter operand, size_t coeff_count, const Modulus &modulus)
        {
            ...
            uint64_t result = 0;
            SIGMA_ITERATE(operand, coeff_count, [&](auto I) {
            ...
            ConstCoeffIter poly, size_t coeff_count, size_t shift, const Modulus &modulus, CoeffIter result)
        {
            ...
            uint64_t coeff_count_mod_mask = static_cast<uint64_t>(coeff_count) - 1;//12位1
