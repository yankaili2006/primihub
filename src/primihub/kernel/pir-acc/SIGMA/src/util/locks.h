...
namespace sigma
{
    namespace util
    {
        ...
        class SIGMA_NODISCARD ReaderWriterLocker
        {
        public:
            ReaderWriterLocker() = default;

            SIGMA_NODISCARD inline ReaderLock acquire_read()
            {
                return ReaderLock(rw_lock_mutex_);
            }

            SIGMA_NODISCARD inline WriterLock acquire_write()
            {
                return WriterLock(rw_lock_mutex_);
            }

            SIGMA_NODISCARD inline ReaderLock try_acquire_read() noexcept
            {
                return ReaderLock(rw_lock_mutex_, std::try_to_lock);
            }

            SIGMA_NODISCARD inline WriterLock try_acquire_write() noexcept
            {
...
namespace sigma
{
    namespace util
    {
        ...
        class SIGMA_NODISCARD ReaderLock
        {
        public:
            ...
            SIGMA_NODISCARD inline bool owns_lock() const noexcept
            {
        ...
        class SIGMA_NODISCARD WriterLock
        {
        public:
        ...
        public:
            ...
            SIGMA_NODISCARD inline ReaderLock acquire_read() noexcept
            {
                return ReaderLock(*this);
            }

            SIGMA_NODISCARD inline WriterLock acquire_write() noexcept
            {
        ...
        SIGMA_NODISCARD inline bool ReaderLock::try_acquire(ReaderWriterLocker &locker) noexcept
        {
            unlock();
            locker.reader_locks_.fetch_add(1, std::memory_order_acquire);
            locker_ = &locker;
        ...
        SIGMA_NODISCARD inline bool WriterLock::try_acquire(ReaderWriterLocker &locker) noexcept
        {
            unlock();
            bool expected = false;
