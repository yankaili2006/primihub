...
namespace sigma
{
    namespace util
    {
        ...
        SIGMA_NODISCARD inline unsigned char add_uint64_generic(
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        SIGMA_NODISCARD inline unsigned char add_uint64(
            T operand1, S operand2, unsigned char carry, unsigned long long *result)
        {
            return SIGMA_ADD_CARRY_UINT64(operand1, operand2, carry, result);
        }

        template <typename T, typename S, typename R, typename = std::enable_if_t<is_uint64_v<T, S, R>>>
        SIGMA_NODISCARD inline unsigned char add_uint64(T operand1, S operand2, R *result)
        {
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        SIGMA_NODISCARD inline unsigned char sub_uint64_generic(
        ...
        template <typename T, typename S, typename = std::enable_if_t<is_uint64_v<T, S>>>
        SIGMA_NODISCARD inline unsigned char sub_uint64(
            T operand1, S operand2, unsigned char borrow, unsigned long long *result)
        {
            return SIGMA_SUB_BORROW_UINT64(operand1, operand2, borrow, result);
        }

        template <typename T, typename S, typename R, typename = std::enable_if_t<is_uint64_v<T, S, R>>>
        SIGMA_NODISCARD inline unsigned char sub_uint64(T operand1, S operand2, R *result)
        {
            *result = operand1 - operand2;
        ...
        {
            SIGMA_MULTIPLY_UINT64(operand1, operand2, result128);
        }
        ...
        SIGMA_FORCE_INLINE void multiply_accumulate_uint64(
        ...
        template <>
        SIGMA_FORCE_INLINE void multiply_accumulate_uint64<0>(
            SIGMA_MAYBE_UNUSED const std::uint64_t *operand1, SIGMA_MAYBE_UNUSED const std::uint64_t *operand2,
            SIGMA_MAYBE_UNUSED unsigned long long *accumulator)
        {
...
#endif
            SIGMA_DIVIDE_UINT128_UINT64(numerator, denominator, quotient);
        }

        void divide_uint192_inplace(std::uint64_t *numerator, std::uint64_t denominator, std::uint64_t *quotient);

        SIGMA_NODISCARD std::uint64_t exponentiate_uint_safe(std::uint64_t operand, std::uint64_t exponent);

        SIGMA_NODISCARD std::uint64_t exponentiate_uint(std::uint64_t operand, std::uint64_t exponent);
    } // namespace util
} // namespace sigma
