...
namespace sigma
{
    namespace util
    {
              ...
              item_count_(MemoryPool::first_alloc_count), first_item_(nullptr)
        {
            ...
            try
            {
                new_alloc.data_ptr = SIGMA_MALLOC(mul_safe(MemoryPool::first_alloc_count, item_byte_count_));
            }
        ...
        MemoryPoolHeadMT::~MemoryPoolHeadMT() noexcept
        {
            ...
            if (clear_on_destruction_)
            {
                ...
                for (auto &alloc : allocs_)
                {
                    ...
                    sigma_memzero(alloc.data_ptr, curr_alloc_byte_count);

                    // Delete this allocation
                    SIGMA_FREE(alloc.data_ptr);
                }
            }
            else
            {
        ...
        MemoryPoolItem *MemoryPoolHeadMT::get()
        {
            ...
            if (old_first == nullptr)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        new_alloc.data_ptr = SIGMA_MALLOC(new_alloc_byte_count);
                    }
                    catch (const bad_alloc &)
                    {
                        // Allocation failed; rethrow
        ...
        Pointer<sigma_byte> MemoryPoolMT::get_for_byte_count(size_t byte_count)
        {
            ...
            while (start < end)
            {
                ...
                {
                    return Pointer<sigma_byte>(mid_head);
                }
            }
            reader_lock.unlock();
            ...
            return Pointer<sigma_byte>(new_head);
        }
        ...
        Pointer<sigma_byte> MemoryPoolST::get_for_byte_count(size_t byte_count)
        {
