...
namespace sigma
{
    namespace util
    {
        SIGMA_NODISCARD inline std::vector<int> naf(int value)
        {
            std::vector<int> res;
        ...
        SIGMA_NODISCARD inline std::uint64_t gcd(std::uint64_t x, std::uint64_t y)
        {
#ifdef SIGMA_DEBUG
            if (x == 0)
            {
        ...
        SIGMA_NODISCARD inline auto xgcd(std::uint64_t x, std::uint64_t y)
        ...
        SIGMA_NODISCARD inline bool are_coprime(std::uint64_t x, std::uint64_t y) noexcept
        {
            return !(gcd(x, y) > 1);
        }

        SIGMA_NODISCARD std::vector<std::uint64_t> multiplicative_orders(
            std::vector<std::uint64_t> conjugate_classes, std::uint64_t modulus);

        SIGMA_NODISCARD std::vector<std::uint64_t> conjugate_classes(
        ...
        SIGMA_NODISCARD auto decompose_babystep_giantstep(
        ...
        SIGMA_NODISCARD bool is_prime(const Modulus &modulus, std::size_t num_rounds = 40);

        // Generate a vector of primes with "bit_size" bits that are congruent to 1 modulo "factor"
        SIGMA_NODISCARD std::vector<Modulus> get_primes(std::uint64_t factor, int bit_size, std::size_t count);

        // Generate one prime with "bit_size" bits that are congruent to 1 modulo "factor"
        SIGMA_NODISCARD inline Modulus get_prime(std::uint64_t factor, int bit_size)
        {
