    ...
    namespace util
    {
        ...
        public:
...
#endif
                SIGMA_ITERATE(iter(operand, modulus, result), coeff_modulus_size, [&](auto I) {
                ...
                PolyIter result) const
            {
                ...
                SIGMA_ITERATE(iter(operand, result), size, [&](auto I) {
...
#endif
                SIGMA_ITERATE(iter(operand, result), coeff_modulus_size, [&](auto I) {
            ...
            */
            SIGMA_NODISCARD std::uint32_t get_elt_from_step(int step) const;

            /**
            Compute the Galois elements corresponding to a vector of given rotation steps.
            */
            SIGMA_NODISCARD std::vector<std::uint32_t> get_elts_from_steps(const std::vector<int> &steps) const;

            /**
            Compute a vector of all necessary galois_elts.
            */
            SIGMA_NODISCARD std::vector<std::uint32_t> get_elts_all() const noexcept;

            /**
            Compute the index in the range of 0 to (coeff_count_ - 1) of a given Galois element.
            */
            SIGMA_NODISCARD static inline std::size_t GetIndexFromElt(std::uint32_t galois_elt)
            {
#ifdef SIGMA_DEBUG
                if (!(galois_elt & 1))
                {
            ...
            void cu_apply_galois(uint64_t* operand, std::size_t coeff_modulus_size, std::uint32_t galois_elt,
