...
namespace sigma
{
    namespace util
    {
        ...
        template <typename... Ts>
        using sigma_void_t = typename VoidType<Ts...>::type;

        template <typename ForwardIt, typename Size, typename Func>
        inline ForwardIt sigma_for_each_n(ForwardIt first, Size size, Func &&func)
        {
            for (; size--; (void)++first)
            {
                func(*first);
        ...
        inline decltype(auto) sigma_apply_impl(Func &&func, Tuple &&tp, std::index_sequence<Is...>)
        {
            return func(std::get<Is>(std::forward<Tuple>(tp))...);
        }

        template <typename Func, typename Tuple, std::size_t... Is>
        inline decltype(auto) sigma_apply(Func &&func, Tuple &&tp)
        {
            using iseq_t = std::make_index_sequence<std::tuple_size<std::decay_t<Tuple>>::value>;
            return sigma_apply_impl(std::forward<Func>(func), std::forward<Tuple>(tp), iseq_t{});
        }
        ...
        SIGMA_NODISCARD inline constexpr bool unsigned_lt(T in1, S in2) noexcept
        {
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
            typename = std::enable_if_t<std::is_integral<S>::value>>
        SIGMA_NODISCARD inline constexpr bool unsigned_leq(T in1, S in2) noexcept
        {
            return static_cast<std::uint64_t>(in1) <= static_cast<std::uint64_t>(in2);
        }
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
            typename = std::enable_if_t<std::is_integral<S>::value>>
        SIGMA_NODISCARD inline constexpr bool unsigned_gt(T in1, S in2) noexcept
        {
            return static_cast<std::uint64_t>(in1) > static_cast<std::uint64_t>(in2);
        }
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
            typename = std::enable_if_t<std::is_integral<S>::value>>
        SIGMA_NODISCARD inline constexpr bool unsigned_geq(T in1, S in2) noexcept
        {
            return static_cast<std::uint64_t>(in1) >= static_cast<std::uint64_t>(in2);
        }
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
            typename = std::enable_if_t<std::is_integral<S>::value>>
        SIGMA_NODISCARD inline constexpr bool unsigned_eq(T in1, S in2) noexcept
        {
            return static_cast<std::uint64_t>(in1) == static_cast<std::uint64_t>(in2);
        }
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_integral<T>::value>,
            typename = std::enable_if_t<std::is_integral<S>::value>>
        SIGMA_NODISCARD inline constexpr bool unsigned_neq(T in1, S in2) noexcept
        {
            return static_cast<std::uint64_t>(in1) != static_cast<std::uint64_t>(in2);
        }

        template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
        SIGMA_NODISCARD inline constexpr T mul_safe(T in1) noexcept
        {
            return in1;
        }

        template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
        SIGMA_NODISCARD inline constexpr T mul_safe(T in1, T in2)
        {
            SIGMA_IF_CONSTEXPR(std::is_unsigned<T>::value)
            {
...
#if (SIGMA_COMPILER == SIGMA_COMPILER_MSVC) && !defined(SIGMA_USE_IF_CONSTEXPR)
#pragma warning(push)
...
#pragma warning(pop)
        ...
        SIGMA_NODISCARD inline constexpr T mul_safe(T in1, T in2, Args &&... args)
        {
            return mul_safe(mul_safe(in1, in2), mul_safe(std::forward<Args>(args)...));
        }

        template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr T add_safe(T in1) noexcept
        {
            return in1;
        }

        template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr T add_safe(T in1, T in2)
        {
            SIGMA_IF_CONSTEXPR(std::is_unsigned<T>::value)
            {
        ...
        SIGMA_NODISCARD inline constexpr T add_safe(T in1, T in2, Args &&... args)
        {
            return add_safe(add_safe(in1, in2), add_safe(std::forward<Args>(args)...));
        }

        template <typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline T sub_safe(T in1, T in2)
        {
            SIGMA_IF_CONSTEXPR(std::is_unsigned<T>::value)
            {
                if (in1 < in2)
        ...
        SIGMA_NODISCARD inline constexpr bool fits_in(S value SIGMA_MAYBE_UNUSED) noexcept
        {
            bool result = false;

            SIGMA_IF_CONSTEXPR(std::is_same<T, S>::value)
            {
                // Same type
                result = true;
            }
            else SIGMA_IF_CONSTEXPR(sizeof(S) <= sizeof(T))
            {
                // Converting to bigger type
                SIGMA_IF_CONSTEXPR(std::is_integral<T>::value && std::is_integral<S>::value)
                {
                    ...
                    else SIGMA_IF_CONSTEXPR(std::is_unsigned<T>::value && std::is_signed<S>::value)
                    {
                        // Converting from signed to at least equally big unsigned type
                        result = value >= 0;
                    }
                }
                else SIGMA_IF_CONSTEXPR(std::is_floating_point<T>::value && std::is_floating_point<S>::value)
                {
                    // Both floating-point
                    result = true;
                }
            ...
            else SIGMA_IF_CONSTEXPR(std::is_floating_point<T>::value)
            {
        ...
        template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr bool sum_fits_in(Args &&... args)
        {
            return fits_in<T>(add_safe(std::forward<Args>(args)...));
        }

        template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr bool sum_fits_in(T in1, Args &&... args)
        {
            return fits_in<T>(add_safe(in1, std::forward<Args>(args)...));
        }

        template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr bool product_fits_in(Args &&... args)
        {
            return fits_in<T>(mul_safe(std::forward<Args>(args)...));
        }

        template <typename T, typename... Args, typename = std::enable_if_t<std::is_arithmetic<T>::value>>
        SIGMA_NODISCARD inline constexpr bool product_fits_in(T in1, Args &&... args)
        {
        ...
        template <
            typename T, typename S, typename = std::enable_if_t<std::is_arithmetic<T>::value>,
            typename = std::enable_if_t<std::is_arithmetic<S>::value>>
        SIGMA_NODISCARD inline T safe_cast(S value)
        {
            SIGMA_IF_CONSTEXPR(!std::is_same<T, S>::value)
            {
                if (!fits_in<T>(value))
        ...
        SIGMA_NODISCARD inline constexpr int hamming_weight(unsigned char value)
        {
        ...
        SIGMA_NODISCARD inline constexpr T reverse_bits(T operand) noexcept
        {
            SIGMA_IF_CONSTEXPR(is_uint32_v<T>)
            {
            ...
            else SIGMA_IF_CONSTEXPR(is_uint64_v<T>)
            {
// Temporarily disable UB warnings when `if constexpr` is not available.
#ifndef SIGMA_USE_IF_CONSTEXPR
#if (SIGMA_COMPILER == SIGMA_COMPILER_MSVC)
#pragma warning(push)
#pragma warning(disable : 4293)
#elif (SIGMA_COMPILER == SIGMA_COMPILER_GCC)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wshift-count-overflow"
#elif (SIGMA_COMPILER == SIGMA_COMPILER_CLANG)
...
#pragma clang diagnostic pop
        ...
        template <typename T, typename = std::enable_if_t<is_uint32_v<T> || is_uint64_v<T>>>
        SIGMA_NODISCARD inline T reverse_bits(T operand, int bit_count)
        {
#ifdef SIGMA_DEBUG
            if (bit_count < 0 ||
        ...
        SIGMA_NODISCARD inline int get_significant_bit_count(std::uint64_t value)
        {
            if (value == 0)
            {
                return 0;
            ...
            SIGMA_MSB_INDEX_UINT64(&result, value);
            return static_cast<int>(result + 1);
        }

        SIGMA_NODISCARD inline bool is_hex_char(char hex)
        {
            if (hex >= '0' && hex <= '9')
            {
                return true;
        ...
        SIGMA_NODISCARD inline char nibble_to_upper_hex(int nibble)
        {
#ifdef SIGMA_DEBUG
        ...
        SIGMA_NODISCARD inline int hex_to_nibble(char hex)
        {
            if (hex >= '0' && hex <= '9')
            {
        ...
        SIGMA_NODISCARD inline int get_hex_string_bit_count(const char *hex_string, int char_count)
        {
#ifdef SIGMA_DEBUG
        ...
        template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
        SIGMA_NODISCARD inline T divide_round_up(T value, T divisor)
        {
#ifdef SIGMA_DEBUG
            if (value < 0)
            {
        ...
        SIGMA_NODISCARD inline bool are_close(T value1, T value2) noexcept
        {
        ...
        template <typename T, typename = std::enable_if_t<std::is_integral<T>::value>>
        SIGMA_NODISCARD inline constexpr bool is_zero(T value) noexcept
        {
            return value == T{ 0 };
        }

        void sigma_memzero(void *data, std::size_t size);
    } // namespace util
} // namespace sigma
