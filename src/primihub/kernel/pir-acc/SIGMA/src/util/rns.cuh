...
namespace sigma
{
    namespace util
    {
        ...
        public:
            ...
            SIGMA_NODISCARD inline const Modulus &operator[](std::size_t index) const
            {
                if (index >= size_)
                {
            ...
            SIGMA_NODISCARD bool contains(const Modulus &value) const noexcept;

            SIGMA_NODISCARD bool is_subbase_of(const RNSBase &superbase) const noexcept;

            SIGMA_NODISCARD inline bool is_superbase_of(const RNSBase &subbase) const noexcept
            {
                return subbase.is_subbase_of(*this);
            }

            SIGMA_NODISCARD inline bool is_proper_subbase_of(const RNSBase &superbase) const noexcept
            {
                return (size_ < superbase.size_) && is_subbase_of(superbase);
            }

            SIGMA_NODISCARD inline bool is_proper_superbase_of(const RNSBase &subbase) const noexcept
            {
                return (size_ > subbase.size_) && !is_subbase_of(subbase);
            }

            SIGMA_NODISCARD RNSBase extend(const Modulus &value) const;

            SIGMA_NODISCARD RNSBase extend(const RNSBase &other) const;

            SIGMA_NODISCARD RNSBase drop() const;

            SIGMA_NODISCARD RNSBase drop(const Modulus &value) const;
            ...
            SIGMA_NODISCARD inline const Modulus *base() const noexcept
            {
                return base_.get();
            }

            SIGMA_NODISCARD inline const std::uint64_t *base_prod() const noexcept
            {
                return base_prod_.get();
            }

            SIGMA_NODISCARD inline const std::uint64_t *punctured_prod_array() const noexcept
            {
                return punctured_prod_array_.get();
            }

            SIGMA_NODISCARD inline const MultiplyUIntModOperand *inv_punctured_prod_mod_base_array() const noexcept
            {
        ...
        public:
            ...
            SIGMA_NODISCARD inline std::size_t ibase_size() const noexcept
            {
                return ibase_.size();
            }

            SIGMA_NODISCARD inline std::size_t obase_size() const noexcept
            {
                return obase_.size();
            }

            SIGMA_NODISCARD inline const RNSBase &ibase() const noexcept
            {
                return ibase_;
            }

            SIGMA_NODISCARD inline const RNSBase &obase() const noexcept
            {
                return obase_;
            }
        ...
        public:
            ...
            */
            ...
            SIGMA_NODISCARD inline auto inv_q_last_mod_q() const noexcept
            {
                return inv_q_last_mod_q_.get();
            }

            SIGMA_NODISCARD inline auto base_Bsk_ntt_tables() const noexcept
            {
                return base_Bsk_ntt_tables_.get();
            }

            SIGMA_NODISCARD inline auto base_q() const noexcept
            {
                return base_q_.get();
            }

            SIGMA_NODISCARD inline auto base_B() const noexcept
            {
                return base_B_.get();
            }

            SIGMA_NODISCARD inline auto base_Bsk() const noexcept
            {
                return base_Bsk_.get();
            }

            SIGMA_NODISCARD inline auto base_Bsk_m_tilde() const noexcept
            {
                return base_Bsk_m_tilde_.get();
            }

            SIGMA_NODISCARD inline auto base_t_gamma() const noexcept
            {
                return base_t_gamma_.get();
            }

            SIGMA_NODISCARD inline auto &m_tilde() const noexcept
            {
                return m_tilde_;
            }

            SIGMA_NODISCARD inline auto &m_sk() const noexcept
            {
                return m_sk_;
            }

            SIGMA_NODISCARD inline auto &t() const noexcept
            {
                return t_;
            }

            SIGMA_NODISCARD inline auto &gamma() const noexcept
            {
                return gamma_;
            }

            SIGMA_NODISCARD inline auto &inv_q_last_mod_t() const noexcept
            {
                return inv_q_last_mod_t_;
            }

            SIGMA_NODISCARD inline const uint64_t &q_last_mod_t() const noexcept
            {
                return q_last_mod_t_;
            }

            void divideAndRoundqLastInplace(uint64_t* input) const;
