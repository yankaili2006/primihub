...
namespace sigma
{
    ...
    namespace util
    {
        ...
        In this file we define a set of custom iterator classes ("SIGMA iterators") that are used throughout Microsoft
        SIGMA for easier iteration over ciphertext polynomials, their RNS components, and the coefficients in the RNS
        components. All SIGMA iterators satisfy the C++ LegacyRandomAccessIterator requirements. SIGMA iterators are ideal
        to use with the SIGMA_ITERATE macro, which expands to std::for_each_n in C++17 and to sigma::util::sigma_for_each_n
        in C++14. All SIGMA iterators derive from SIGMAIterBase.

        The most important SIGMA iterator classes behave as illustrated by the following diagram:
        ...
        PtrIter<T *> and StrideIter<T *> are both templated SIGMA iterators that wrap raw pointers. The difference
        ...
        @par IterTuple
        An extremely useful template class is the (variadic) IterTuple<...> that allows multiple SIGMA iterators to be
        zipped together. An IterTuple is itself a SIGMA iterator and nested IterTuple types are used commonly in the
        ...
        The individual components of an IterTuple can be accessed with the sigma::util::get<i>(...) functions. The
        ...
        @par Creating SIGMA iterators
        Iterators are easiest to create using the variadic iter function that, when given one or more arguments that can
        naturally be converted to SIGMA iterators, outputs an appropriate iterator, or iterator tuple. Consider again the
        ...
        passing a variadic set of constructor arguments; iter will output an IterTuple consisting of SIGMA iterators
        ...
        In addition to the iterator types described above, we provide ReverseIter<SIGMAIter> that reverses the direction
        of iteration. ReverseIter<SIGMAIter> dereferences to the same type as SIGMAIter: for example, dereferencing
        ReverseIter<RNSIter> results in CoeffIter, not ReverseIter<CoeffIter>.

        It is easy to create a ReverseIter from a given SIGMA iterator using the function reverse_iter. For example,
        ...
        @par SIGMA_ITERATE
        SIGMA iterators are made to be used with the SIGMA_ITERATE macro to iterate over a certain number of steps, and
        for each step call a given lambda function. In C++17 SIGMA_ITERATE expands to std::for_each_n, and in C++14 it
        expands to sigma::util::sigma_for_each_n -- a custom implementation. For example, the following snippet appears
        ...
        arguments and converts each RNS component separately. Looking at sigma/util/ntt.h we see that the function
        ntt_negacyclic_harvey_lazy is again implemented using SIGMA_ITERATE. Specifically, it contains the following:
        ...
        There are two important coding conventions in the above code snippets that are to be observed:
            ...
            1. Use I, J, K, ... for the lambda function parameters representing SIGMA iterators. This is compact and
               makes it very clear that the objects in question are SIGMA iterators since such variable names should not
               be used in SIGMA in any other context.
            2. Lambda functions passed to SIGMA_ITERATE should almost always (see 3.) take a parameter of type auto. This
               will produce simple looking code that performs well with the expected outcome.
            3. The only exception to 2. is when SIGMA_ITERATE operates on a single PtrIter<T *>: dereferencing returns a
               T &, which may be important to forward by reference to the lambda function. For an example of this, see
               sigma::util::ntt_negacyclic_harvey in sigma/util/ntt.h, where the lambda function parameter is auto &.
        ...
        sigma::util::negate_poly_coeffmod can negate a single RNS component modulo a given Modulus (CoeffIter overload),
        ...
        Sometimes inside SIGMA_ITERATE lambda functions it is convenient to know the index of the iteration. This can be
        ...
        probably a good idea to call sigma::util::allocate inside the lambda function for any allocations needed, rather
        ...
        SIGMA_ALLOCATE_GET_POLY_ITER(temp, count, poly_modulus_degree, coeff_modulus_size, pool);
        ...
        */
        ...
        template <typename SIGMAIter>
        class ReverseIter;

        template <typename... SIGMAIters>
        class IterTuple;
        ...
        namespace iterator_internal
        {
            ...
            template <typename T>
            struct IterType<
                std::enable_if_t<std::is_base_of<SIGMAIterBase, std::decay_t<T>>::value && !std::is_pointer<T>::value>,
                T>
            {
        ...
        class SeqIter : public SIGMAIterBase
        {
        public:
            ...
            SIGMA_NODISCARD inline reference operator*() const noexcept
            {
                return value_;
            }

            template <typename SizeT>
            SIGMA_NODISCARD inline value_type operator[](SizeT n) const noexcept
            {
            ...
            template <typename SizeT>
            SIGMA_NODISCARD inline self_type operator+(SizeT n) const noexcept
            {
                return value_ + static_cast<T>(n);
            }
            ...
            SIGMA_NODISCARD inline difference_type operator-(const self_type &b) const noexcept
            {
                return static_cast<difference_type>(value_) - static_cast<difference_type>(b.value_);
            }

            SIGMA_NODISCARD inline bool operator==(const self_type &compare) const noexcept
            {
                return value_ == *compare;
            }

            template <typename S, typename = std::enable_if_t<std::is_arithmetic<S>::value>>
            SIGMA_NODISCARD inline bool operator==(S compare) const noexcept
            {
                return value_ == compare;
            }
            ...
            SIGMA_NODISCARD inline operator T() const noexcept
            {
                return value_;
            }
        ...
        template <typename T>
        SIGMA_NODISCARD inline auto seq_iter(T value = 0) -> SeqIter<T>
        {
            return value;
        }

        template <typename T>
        class PtrIter<T *> : public SIGMAIterBase
        {
        public:
            ...
            template <typename SizeT>
            SIGMA_NODISCARD inline reference operator[](SizeT n) const noexcept
            {
                return ptr_[n];
            }
            ...
            template <typename S>
            SIGMA_NODISCARD inline difference_type operator-(const PtrIter<S *> &b) const noexcept
            {
                return std::distance(b.ptr(), ptr_);
            }

            template <typename S>
            SIGMA_NODISCARD inline bool operator==(const PtrIter<S *> &compare) const noexcept
            {
            ...
            SIGMA_NODISCARD explicit inline operator bool() const noexcept
            {
                return nullptr != ptr_;
            }
            ...
            SIGMA_NODISCARD inline pointer ptr() const noexcept
            {
                return ptr_;
            }

            SIGMA_NODISCARD inline operator pointer() const noexcept
            {
                return ptr_;
            }
        ...
        template <typename T>
        class StrideIter<T *> : public SIGMAIterBase
        {
        public:
            ...
            template <typename S>
            SIGMA_NODISCARD inline difference_type operator-(const StrideIter<S *> &b) const
            {
#ifdef SIGMA_DEBUG
                if (!stride_)
                {
            ...
            template <typename S>
            SIGMA_NODISCARD inline bool operator==(const StrideIter<S *> &compare) const noexcept
            {
            ...
            SIGMA_NODISCARD inline std::size_t stride() const noexcept
            {
                return stride_;
            }
        ...
        class RNSIter : public SIGMAIterBase
        {
        public:
            ...
            SIGMA_NODISCARD inline difference_type operator-(const self_type &b) const
            {
#ifdef SIGMA_DEBUG
                if (!step_size_)
                {
            ...
            SIGMA_NODISCARD inline operator std::uint64_t *() const noexcept
            {
        ...
        class ConstRNSIter : public SIGMAIterBase
        {
        public:
            ...
            SIGMA_NODISCARD inline operator const std::uint64_t *() const noexcept
            {
        ...
        class PolyIter : public SIGMAIterBase
        {
        public:
        ...
        class ConstPolyIter : public SIGMAIterBase
        {
        public:
        ...
        template <typename SIGMAIter>
        class ReverseIter : public SIGMAIter
        {
        public:
            static_assert(
                std::is_base_of<SIGMAIterBase, SIGMAIter>::value,
                "Template parameter must derive from sigma::util::SIGMAIterBase");

            using self_type = ReverseIter<SIGMAIter>;

            // Standard iterator typedefs
            using value_type = typename std::iterator_traits<SIGMAIter>::value_type;
            using pointer = typename std::iterator_traits<SIGMAIter>::pointer;
            using reference = typename std::iterator_traits<SIGMAIter>::reference;
            using iterator_category = typename std::iterator_traits<SIGMAIter>::iterator_category;
            using difference_type = typename std::iterator_traits<SIGMAIter>::difference_type;

            ReverseIter() : SIGMAIter()
            {}

            ReverseIter(const SIGMAIter &copy) : SIGMAIter(copy)
            {}
            ...
            SIGMA_NODISCARD inline difference_type operator-(const self_type &b) const
            {
                ...
                return static_cast<SIGMAIter>(*b) - static_cast<SIGMAIter>(*this);
            }

            SIGMA_NODISCARD inline bool operator<(const self_type &compare) const noexcept
            {
                // Note the reversed order
                return static_cast<SIGMAIter>(*this) > static_cast<SIGMAIter>(*compare);
            }
        ...
        namespace iterator_internal
        {
            ...
            template <typename SIGMAIter, typename... Rest>
            struct extend_iter_tuple<SIGMAIter, IterTuple<Rest...>>
            {
                using type = IterTuple<SIGMAIter, Rest...>;
            };

            template <typename SIGMAIter1, typename SIGMAIter2>
            struct extend_iter_tuple<SIGMAIter1, SIGMAIter2>
            {
                using type = IterTuple<SIGMAIter1, SIGMAIter2>;
            };
        ...
        template <typename SIGMAIter, typename... Rest>
        class IterTuple<SIGMAIter, Rest...> : public SIGMAIterBase
        {
        public:
            static_assert(
                std::is_base_of<SIGMAIterBase, SIGMAIter>::value,
            ...
            using self_type = IterTuple<SIGMAIter, Rest...>;

            // Standard iterator typedefs
            using value_type = typename iterator_internal::extend_std_tuple<
                typename std::iterator_traits<SIGMAIter>::value_type,
            ...
            IterTuple(SIGMAIter first, IterTuple<Rest...> rest) : first_(first), rest_(rest){};

            IterTuple(SIGMAIter first, Rest... rest) : first_(first), rest_(rest...)
            {}

            template <typename... Ts>
            IterTuple(const std::tuple<Ts...> &tp)
                : IterTuple(sigma_apply(
                      [](auto &&... args) -> IterTuple { return { std::forward<decltype(args)>(args)... }; },
                      std::forward<decltype(tp)>(tp)))
            {
                static_assert(
                    sizeof...(Ts) == sizeof...(Rest) + 1, "std::tuple size does not match sigma::util::IterTuple size");
            }
            ...
            SIGMA_NODISCARD inline value_type operator*() const noexcept
            {
                return sigma_apply(
            ...
            SIGMA_NODISCARD inline bool operator<(const self_type &compare) const
            {
            ...
            SIGMA_NODISCARD inline const SIGMAIter &first() const noexcept
            {
                return first_;
            }

            SIGMA_NODISCARD inline const IterTuple<Rest...> &rest() const noexcept
            {
                return rest_;
            }
        ...
        template <typename SIGMAIter>
        class IterTuple<SIGMAIter> : public SIGMAIterBase
        {
        public:
            static_assert(
                std::is_base_of<SIGMAIterBase, SIGMAIter>::value,
            ...
            using self_type = IterTuple<SIGMAIter>;

            // Standard iterator typedefs
            using value_type = std::tuple<typename std::iterator_traits<SIGMAIter>::value_type>;
            using pointer = void;
            using reference = const value_type &;
            using iterator_category = std::random_access_iterator_tag;
            using difference_type = std::ptrdiff_t;
            ...
            IterTuple(SIGMAIter first) : first_(first)
            {}
        ...
        // Out-of-class operator+ for all SIGMA iterators
        template <
            typename SizeT, typename SIGMAIter,
            typename = std::enable_if_t<std::is_base_of<SIGMAIterBase, SIGMAIter>::value>>
        SIGMA_NODISCARD inline SIGMAIter operator+(SizeT n, SIGMAIter it)
        {
            return it + n;
        }
        ...
        namespace iterator_internal
        {
            ...
            {
                template <typename SIGMAIter, typename... Rest>
                SIGMA_NODISCARD static auto Apply(const IterTuple<SIGMAIter, Rest...> &it) noexcept
                {
            ...
            template <std::size_t N, typename... SIGMAIters>
            SIGMA_NODISCARD inline auto get(const IterTuple<SIGMAIters...> &it) noexcept
            {
                static_assert(N < sizeof...(SIGMAIters), "sigma::util::IterTuple index out of range");
        ...
        template <
            std::size_t N, std::size_t... Rest, typename... SIGMAIters, typename = std::enable_if_t<sizeof...(Rest)>>
        SIGMA_NODISCARD inline auto get(const IterTuple<SIGMAIters...> &it) noexcept
        {
        ...
        SIGMA_NODISCARD inline auto get(const std::tuple<Ts...> &tp) noexcept
        {
            return get<Rest...>(std::get<N>(tp));
        }

        template <typename... Ts>
        SIGMA_NODISCARD inline auto iter(Ts &&... ts) noexcept -> typename iterator_internal::IterType<void, Ts...>::type
        {
            return { std::forward<Ts>(ts)... };
        }

        template <typename... Ts>
        SIGMA_NODISCARD inline auto reverse_iter(Ts &&... ts) noexcept
