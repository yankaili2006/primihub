...
namespace sigma
{
    namespace util
    {
        ...
        {
            bool result = false;
            SIGMA_ITERATE(iter(base_), size_, [&](auto &I) { result = result || (I == value); });
            return result;
        }
        ...
        {
            bool result = true;
            SIGMA_ITERATE(iter(base_), size_, [&](auto &I) { result = result && superbase.contains(I); });
            return result;
        }
        ...
        RNSBase RNSBase::extend(const Modulus &value) const
        {
            ...
            SIGMA_ITERATE(iter(base_), size_, [&](auto I) {
                // The base must be coprime
        ...
        bool RNSBase::initialize()
        {
            ...
            if (size_ > 1)
            {
                ...
                SIGMA_ITERATE(iter(base_, rnsbase_values), size_, [&](auto I) { get<1>(I) = get<0>(I).value(); });

                // Create punctured products
                StrideIter<uint64_t *> punctured_prod(punctured_prod_array_.get(), size_);
                SIGMA_ITERATE(iter(punctured_prod, size_t(0)), size_, [&](auto I) {
                ...
                SIGMA_ITERATE(iter(punctured_prod, base_, inv_punctured_prod_mod_base_array_), size_, [&](auto I) {
        ...
        void RNSBase::decompose(uint64_t *value, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                SIGMA_ITERATE(iter(value, base_), size_, [&](auto I) {
        ...
        void RNSBase::decompose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                SIGMA_ALLOCATE_GET_STRIDE_ITER(value_copy, uint64_t, count, size_, pool);
                ...
                SIGMA_ITERATE(iter(base_, value_out), size_, [&](auto I) {
                    // For each multi-precision integer in value_copy ...
                    SIGMA_ITERATE(iter(get<1>(I), value_copy), count, [&](auto J) {
        ...
        void RNSBase::compose_array(uint64_t *value, size_t count, MemoryPoolHandle pool) const
        {
            ...
            if (size_ > 1)
            {
                ...
                auto temp_mpi(allocate_uint(size_, pool));
                SIGMA_ITERATE(iter(temp_array_iter, value_iter), count, [&](auto I) {
                    SIGMA_ITERATE(
        ...
        void BaseConverter::fast_convert(ConstCoeffIter in, CoeffIter out, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_COEFF_ITER(temp, ibase_size, pool);
            SIGMA_ITERATE(
            ...
            SIGMA_ITERATE(iter(out, base_change_matrix_, obase_.base()), obase_size, [&](auto I) {
        ...
        void BaseConverter::fast_convert_array(ConstRNSIter in, RNSIter out, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_STRIDE_ITER(temp, uint64_t, count, ibase_size, pool);
            ...
            SIGMA_ITERATE(
                ...
                [&](auto I) {
                    ...
                    if (get<1>(I).operand == 1)
                    {
                        ...
                        SIGMA_ITERATE(iter(get<0>(I), temp), count, [&](auto J) {
        ...
        void BaseConverter::exact_convert_array(ConstRNSIter in, CoeffIter out, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_STRIDE_ITER(v, double_t, count, ibase_size, pool);

            // Aggregated rounded v
            SIGMA_ALLOCATE_GET_PTR_ITER(aggregated_rounded_v, uint64_t, count, pool);
            ...
            SIGMA_ITERATE(
                ...
                [&](auto I) {
                    ...
                    {
                        // No multiplication needed
                        SIGMA_ITERATE(iter(get<0>(I), temp, v), count, [&](auto J) {
                            // Reduce modulo ibase element
                            get<1>(J)[ibase_index] = barrett_reduce_64(get<0>(J), get<2>(I));
            ...
            SIGMA_ITERATE(iter(v, aggregated_rounded_v), count, [&](auto I) {
            ...
            SIGMA_ITERATE(iter(out, temp, aggregated_rounded_v), count, [&](auto J) {
                // Compute the base conversion sum modulo obase element
        ...
        void BaseConverter::initialize()
        {
            ...
            SIGMA_ITERATE(iter(base_change_matrix_, obase_.base()), obase_.size(), [&](auto I) {
                ...
                SIGMA_ITERATE(iter(get<0>(I), ibase_punctured_prod_array), ibase_.size(), [&](auto J) {
        ...
        void RNSTool::initialize(size_t poly_modulus_degree, const RNSBase &q, const Modulus &t)
        {
            ...
            if (q.size() < SIGMA_COEFF_MOD_COUNT_MIN || q.size() > SIGMA_COEFF_MOD_COUNT_MAX)
            {
                throw invalid_argument("rnsbase is invalid");
            }
            ...
            if (coeff_count_power < 0 || poly_modulus_degree > SIGMA_POLY_MOD_DEGREE_MAX ||
                poly_modulus_degree < SIGMA_POLY_MOD_DEGREE_MIN)
            {
            ...
            // primes q_i are bounded to be SIGMA_USER_MOD_BIT_COUNT_MAX (60) bits, and all primes in B and m_sk are
            // SIGMA_INTERNAL_MOD_BIT_COUNT (61) bits.
            ...
            if (32 + t_.bit_count() + total_coeff_bit_count >=
                SIGMA_INTERNAL_MOD_BIT_COUNT * safe_cast<int>(base_q_size) + SIGMA_INTERNAL_MOD_BIT_COUNT)
            {
                base_B_size++;
            }
            ...
            auto baseconv_primes =
                get_primes(mul_safe(size_t(2), coeff_count_), SIGMA_INTERNAL_MOD_BIT_COUNT, base_Bsk_m_tilde_size);
            ...
            SIGMA_ITERATE(iter(prod_B_mod_q_, base_q_->base()), base_q_size, [&](auto I) {
            ...
            SIGMA_ITERATE(iter(inv_m_tilde_mod_Bsk_, base_Bsk_->base()), base_Bsk_size, [&](auto I) {
            ...
            SIGMA_ITERATE(iter(prod_q_mod_Bsk_, base_Bsk_->base()), base_Bsk_size, [&](auto I) {
            ...
            if (base_t_gamma_)
            {
                ...
                SIGMA_ITERATE(iter(prod_t_gamma_mod_q_, base_q_->base()), base_q_size, [&](auto I) {
                    get<0>(I).set(
                ...
                SIGMA_ITERATE(iter(neg_inv_q_mod_t_gamma_, base_t_gamma_->base()), base_t_gamma_size, [&](auto I) {
            ...
            SIGMA_ITERATE(iter(inv_q_last_mod_q_, base_q_->base()), base_q_size - 1, [&](auto I) {
        ...
        __global__ void gDivideAndRoundqLastInplace(
                ...
                auto last_input = input + (base_q_size - 1) * coeff_count;//最后一个模数的值
                last_input[tid] = kernel_util::d_add_uint_mod(last_input[tid], half, base_q[base_q_size - 1]);
                for(int i = 0; i < base_q_size - 1; i++){
                    uint64_t temp;
                    const Modulus& b = base_q[i];
                    temp = kernel_util::d_barrett_reduce_64(last_input[tid], b);
                    uint64_t half_mod = kernel_util::d_barrett_reduce_64(half, b);
                    temp = kernel_util::d_sub_uint_mod(temp, half_mod, b);
                    uint64_t temp_result;
                    std::int64_t borrow = kernel_util::d_sub_uint_64(input[i * coeff_count + tid], temp, &temp_result);
                    input[i * coeff_count + tid] = temp_result + (b.value() & static_cast<std::uint64_t>(-borrow));
                    input[i * coeff_count + tid] =
                            kernel_util::d_multiply_uint_mod(input[i * coeff_count + tid], inv_q_last_mod_q[i],b);
            }

        }

        void RNSTool::divideAndRoundqLastInplace(uint64_t* input) const{

            auto size = base_q_->size();
            Modulus* base_q_to_device = KernelProvider::malloc<Modulus>(size);
            KernelProvider::copy(base_q_to_device, base_q_->base(), size);

            MultiplyUIntModOperand* inv_q_to_device = KernelProvider::malloc<MultiplyUIntModOperand>(size);
            KernelProvider::copy(inv_q_to_device, inv_q_last_mod_q_.get(), size);

            uint64_t q_last_half_ = (*base_q())[base_q()->size() - 1].value() >> 1;
            int blocknum = ceil(coeff_count_ / (1024.0));
            gDivideAndRoundqLastInplace<<<blocknum, 1024>>>(input, coeff_count_,
                    base_q_->size(), base_q_to_device,
                    inv_q_to_device, q_last_half_
            );
            KernelProvider::free(base_q_to_device);
            KernelProvider::free(inv_q_to_device);
        }
        ...
        void RNSTool::divide_and_round_q_last_inplace(RNSIter input, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_COEFF_ITER(temp, coeff_count_, pool);
            SIGMA_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base()), base_q_size - 1, [&](auto I) {
                // (ct mod qk) mod qi
            ...
            RNSIter input, ConstNTTTablesIter rns_ntt_tables, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_COEFF_ITER(temp, coeff_count_, pool);
            SIGMA_ITERATE(iter(input, inv_q_last_mod_q_, base_q_->base(), rns_ntt_tables), base_q_size - 1, [&](auto I) {
                // (ct mod qk) mod qi
                ...
                SIGMA_ITERATE(temp, coeff_count_, [&](auto &J) { J += neg_half_mod; });
#if SIGMA_USER_MOD_BIT_COUNT_MAX <= 60
                // Since SIGMA uses at most 60-bit moduli, 8*qi < 2^63.
                ...
                // Note: lambda function parameter must be passed by reference here
                SIGMA_ITERATE(temp, coeff_count_, [&](auto &J) {
                ...
                SIGMA_ITERATE(iter(get<0>(I), temp), coeff_count_, [&](auto J) { get<0>(J) += qi_lazy - get<1>(J); });
        ...
        void RNSTool::fastbconv_sk(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            SIGMA_ALLOCATE_GET_COEFF_ITER(alpha_sk, coeff_count_, pool);
            SIGMA_ITERATE(iter(alpha_sk, temp, input[base_B_size]), coeff_count_, [&](auto I) {
            ...
            SIGMA_ITERATE(iter(prod_B_mod_q_, base_q_->base(), destination), base_q_size, [&](auto I) {
                ...
                SIGMA_ITERATE(iter(alpha_sk, get<2>(I)), coeff_count_, [&](auto J) {
        ...
        void RNSTool::sm_mrq(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            // Compute r_m_tilde
            SIGMA_ALLOCATE_GET_COEFF_ITER(r_m_tilde, coeff_count_, pool);
            ...
            SIGMA_ITERATE(
                ...
                [&](auto I) {
                    ...
                    SIGMA_ITERATE(iter(get<0>(I), r_m_tilde, get<4>(I)), coeff_count_, [&](auto J) {
        ...
        void RNSTool::fast_floor(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            input += base_q_size;
            SIGMA_ITERATE(iter(input, inv_prod_q_mod_Bsk_, base_Bsk_->base(), destination), base_Bsk_size, [&](auto I) {
                SIGMA_ITERATE(iter(get<0>(I), get<3>(I)), coeff_count_, [&](auto J) {
        ...
        void RNSTool::fastbconv_m_tilde(ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool) const
        {
            ...
            */
            ...
            SIGMA_ALLOCATE_GET_RNS_ITER(temp, coeff_count_, base_q_size, pool);
        ...
        void RNSTool::decrypt_scale_and_round(ConstRNSIter input, CoeffIter destination, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_GET_RNS_ITER(temp, coeff_count_, base_q_size, pool);
            SIGMA_ITERATE(iter(input, prod_t_gamma_mod_q_, base_q_->base(), temp), base_q_size, [&](auto I) {
            ...
            SIGMA_ALLOCATE_GET_RNS_ITER(temp_t_gamma, coeff_count_, base_t_gamma_size, pool);
            ...
            // gamma inverse mod t
            SIGMA_ITERATE(iter(temp_t_gamma[0], temp_t_gamma[1], destination), coeff_count_, [&](auto I) {
            ...
            RNSIter input, ConstNTTTablesIter rns_ntt_tables, MemoryPoolHandle pool) const
        {
            ...
            SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(neg_c_last_mod_t, coeff_count_, pool);
            ...
            SIGMA_ALLOCATE_ZERO_GET_COEFF_ITER(delta_mod_q_i, coeff_count_, pool);

            SIGMA_ITERATE(iter(input, curr_modulus, inv_q_last_mod_q_, rns_ntt_tables), modulus_size - 1, [&](auto I) {
                ...
                SIGMA_ITERATE(iter(delta_mod_q_i, c_last), coeff_count_, [&](auto J) {
                    get<0>(J) = add_uint_mod(get<0>(J), barrett_reduce_64(get<1>(J), get<1>(I)), get<1>(I));
                });
                ntt_negacyclic_harvey(delta_mod_q_i, get<3>(I));
                SIGMA_ITERATE(iter(get<0>(I), delta_mod_q_i), coeff_count_, [&](auto J) {
