...
namespace sigma
{
    ...
    void Ciphertext::reserve(const SIGMAContext &context, parms_id_type parms_id, size_t size_capacity)
    {
        // Verify parameters
    ...
    {
        if (size_capacity < SIGMA_CIPHERTEXT_SIZE_MIN || size_capacity > SIGMA_CIPHERTEXT_SIZE_MAX)
        {
...
/*    void Ciphertext::resize_internal(size_t size, size_t poly_modulus_degree, size_t coeff_modulus_size)
    ...
    void Ciphertext::expand_seed(
        const SIGMAContext &context, const UniformRandomGeneratorInfo &prng_info, SIGMAVersion version)
    {
        ...
        {
            sample_poly_uniform_sigma_3_4(prng, context_data_ptr->parms(), data(1));
        }
        else if (version.major == 3 && version.minor == 5)
        {
            sample_poly_uniform_sigma_3_5(prng, context_data_ptr->parms(), data(1));
        }
        else
        {
    ...
    streamoff Ciphertext::save_size(compr_mode_type compr_mode) const
    {
        ...
        size_t members_size = Serialization::ComprSizeEstimate(
            add_safe(
                ...
                sizeof(sigma_byte), // is_ntt_form_
        ...
        return safe_cast<streamoff>(add_safe(sizeof(Serialization::SIGMAHeader), members_size));
    }
    ...
    void Ciphertext::save_members(ostream &stream) const
    {
        ...
        try
        {
            ...
            sigma_byte is_ntt_form_byte = static_cast<sigma_byte>(is_ntt_form_);
            stream.write(reinterpret_cast<const char *>(&is_ntt_form_byte), sizeof(sigma_byte));
            ...
            if (has_seed_marker())
            {
                ...
                info.load(reinterpret_cast<const sigma_byte *>(data(1) + 1), info_size);
    ...
    void Ciphertext::load_members(const SIGMAContext &context, istream &stream, SIGMA_MAYBE_UNUSED SIGMAVersion version)
    {
        // Verify parameters
        if (!context.parameters_set())
        {
        ...
        try
        {
            ...
            sigma_byte is_ntt_form_byte;
            stream.read(reinterpret_cast<char *>(&is_ntt_form_byte), sizeof(sigma_byte));
            uint64_t size64 = 0;
            ...
            new_data.is_ntt_form_ = (is_ntt_form_byte == sigma_byte{}) ? false : true;
