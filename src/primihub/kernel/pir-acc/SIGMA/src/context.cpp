...
using error_type = sigma::EncryptionParameterQualifiers::error_type;
...
namespace sigma
{
    ...
    const char *EncryptionParameterQualifiers::parameter_error_message() const noexcept
    {
        ...
        case error_type::invalid_coeff_modulus_size:
            return "coeff_modulus's primes' count is not bounded by SIGMA_COEFF_MOD_COUNT_MIN(MAX)";

        case error_type::invalid_coeff_modulus_bit_count:
            return "coeff_modulus's primes' bit counts are not bounded by SIGMA_USER_MOD_BIT_COUNT_MIN(MAX)";

        case error_type::invalid_coeff_modulus_no_ntt:
            return "coeff_modulus's primes are not congruent to 1 modulo (2 * poly_modulus_degree)";

        case error_type::invalid_poly_modulus_degree:
            return "poly_modulus_degree is not bounded by SIGMA_POLY_MOD_DEGREE_MIN(MAX)";
        ...
        case error_type::invalid_plain_modulus_bit_count:
            return "plain_modulus's bit count is not bounded by SIGMA_PLAIN_MOD_BIT_COUNT_MIN(MAX)";
    ...
    SIGMAContext::ContextData SIGMAContext::validate(EncryptionParameters parms)
    {
        ...
        if (coeff_modulus.size() > SIGMA_COEFF_MOD_COUNT_MAX || coeff_modulus.size() < SIGMA_COEFF_MOD_COUNT_MIN)
        {
        ...
        for (size_t i = 0; i < coeff_modulus_size; i++)
        {
            ...
            if (coeff_modulus[i].value() >> SIGMA_USER_MOD_BIT_COUNT_MAX ||
                !(coeff_modulus[i].value() >> (SIGMA_USER_MOD_BIT_COUNT_MIN - 1)))
            {
        ...
        if (poly_modulus_degree < SIGMA_POLY_MOD_DEGREE_MIN || poly_modulus_degree > SIGMA_POLY_MOD_DEGREE_MAX)
        {
        ...
        try
        {
            ...
            context_data.device_small_ntt_tables_ = DeviceArray(context_data.small_ntt_tables_.get(), coeff_modulus_size, true);
        }
        catch (const invalid_argument &)
        {
        ...
        if (parms.scheme() == scheme_type::bfv || parms.scheme() == scheme_type::bgv)
        {
            ...
            if (plain_modulus.value() >> SIGMA_PLAIN_MOD_BIT_COUNT_MAX ||
                !(plain_modulus.value() >> (SIGMA_PLAIN_MOD_BIT_COUNT_MIN - 1)))
            {
            ...
            try
            {
                ...
                context_data.device_plain_ntt_tables_ = DeviceArray(context_data.plain_ntt_tables_.get(), coeff_modulus_size, true);
            }
            catch (const invalid_argument &)
            {
    ...
    parms_id_type SIGMAContext::create_next_context_data(const parms_id_type &prev_parms_id)
    {
