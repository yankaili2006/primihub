...
namespace sigma
{
    ...
    */
    void random_bytes(sigma_byte *buf, std::size_t count);

    /**
    Returns a random 64-bit unsigned integer.
    */
    SIGMA_NODISCARD inline std::uint64_t random_uint64()
    {
        std::uint64_t result;
        random_bytes(reinterpret_cast<sigma_byte *>(&result), sizeof(result));
        return result;
    }
    ...
    public:
        ...
        */
        SIGMA_NODISCARD inline bool operator==(const UniformRandomGeneratorInfo &compare) const noexcept
        {
        ...
        */
        void clear() noexcept
        {
            ...
            util::sigma_memzero(seed_.data(), prng_seed_byte_count);
        }
        ...
        */
        ...
        an official Microsoft SIGMA PRNG type, the return value is nullptr.
        */
        ...
        */
        SIGMA_NODISCARD inline bool has_valid_prng_type() const noexcept
        {
            switch (type_)
            {
        ...
        */
        SIGMA_NODISCARD inline prng_type type() const noexcept
        {
            return type_;
        }
        ...
        */
        SIGMA_NODISCARD inline prng_type &type() noexcept
        {
            return type_;
        }
        ...
        */
        SIGMA_NODISCARD inline const prng_seed_type &seed() const noexcept
        {
            return seed_;
        }
        ...
        Returns a reference to the PRNG seed.
        */
        SIGMA_NODISCARD inline prng_seed_type &seed() noexcept
        {
            return seed_;
        }
        ...
        */
        SIGMA_NODISCARD static inline std::streamoff SaveSize(
            compr_mode_type compr_mode = Serialization::compr_mode_default)
        {
            ...
            return static_cast<std::streamoff>(sizeof(Serialization::SIGMAHeader) + members_size);
        }
        ...
        @throws std::logic_error if the size does not fit in the return type
        */
        SIGMA_NODISCARD inline std::streamoff save_size(
        ...
        */
        ...
        @throws std::runtime_error if I/O operations failed
        */
        inline std::streamoff load(const sigma_byte *in, std::size_t size)
        {
            using namespace std::placeholders;
            UniformRandomGeneratorInfo new_info;
            auto in_size = Serialization::Load(
    ...
    public:
        ...
        void load_members(std::istream &stream, SIGMAVersion version);
    ...
    */
    ...
    public:
        ...
        */
        ...
        SIGMA_NODISCARD inline prng_seed_type seed() const noexcept
        {
            prng_seed_type ret{};
        ...
        */
        void generate(std::size_t byte_count, sigma_byte *destination);

        /**
        Generates a new unsigned 32-bit random number.
        */
        SIGMA_NODISCARD inline std::uint32_t generate()
        {
            std::uint32_t result;
            generate(sizeof(result), reinterpret_cast<sigma_byte *>(&result));
            return result;
        }
        ...
        */
        SIGMA_NODISCARD inline UniformRandomGeneratorInfo info() const noexcept
        {
    ...
    protected:
        SIGMA_NODISCARD virtual prng_type type() const noexcept = 0;
    ...
    private:
        DynArray<sigma_byte> buffer_;

        std::mutex mutex_;

    protected:
        sigma_byte *const buffer_begin_;

        sigma_byte *const buffer_end_;

        sigma_byte *buffer_head_;
    };
    ...
    */
    ...
    public:
        ...
        */
        SIGMA_NODISCARD auto create() -> std::shared_ptr<UniformRandomGenerator>
        {
            return use_random_seed_ ? create_impl([]() {
                prng_seed_type seed;
                random_bytes(reinterpret_cast<sigma_byte *>(seed.data()), prng_seed_byte_count);
                return seed;
            }())
        ...
        */
        SIGMA_NODISCARD auto create(prng_seed_type seed) -> std::shared_ptr<UniformRandomGenerator>
        {
        ...
        */
        SIGMA_NODISCARD inline bool use_random_seed() noexcept
        {
        ...
        */
        SIGMA_NODISCARD inline prng_seed_type default_seed() noexcept
        {
            return default_seed_;
        }

    protected:
        SIGMA_NODISCARD virtual auto create_impl(prng_seed_type seed) -> std::shared_ptr<UniformRandomGenerator> = 0;
    ...
    */
    ...
    protected:
        SIGMA_NODISCARD prng_type type() const noexcept override
        {
    ...
    protected:
        SIGMA_NODISCARD auto create_impl(prng_seed_type seed) -> std::shared_ptr<UniformRandomGenerator> override
        {
