...
namespace sigma
{
    ...
    qualifiers are automatically created by the SIGMAContext class, silently passed
    ...
    */
    ...
    public:
        ...
        */
        enum class error_type : int
        {
            ...
            */
            ...
            /**
            coeff_modulus's primes' count is not bounded by SIGMA_COEFF_MOD_COUNT_MIN(MAX)
            */
            invalid_coeff_modulus_size = 2,

            /**
            coeff_modulus's primes' bit counts are not bounded by SIGMA_USER_MOD_BIT_COUNT_MIN(MAX)
            */
            ...
            */
            ...
            /**
            poly_modulus_degree is not bounded by SIGMA_POLY_MOD_DEGREE_MIN(MAX)
            */
            ...
            */
            ...
            /**
            plain_modulus's bit count is not bounded by SIGMA_PLAIN_MOD_BIT_COUNT_MIN(MAX)
            */
        ...
        - success, if parameters are considered valid by Microsoft SIGMA;
        ...
        */
        SIGMA_NODISCARD const char *parameter_error_name() const noexcept;

        /**
        Returns a comprehensive message that interprets parameter_error.
        */
        SIGMA_NODISCARD const char *parameter_error_message() const noexcept;

        /**
        Tells whether parameter_error is error_type::success.
        */
        SIGMA_NODISCARD inline bool parameters_set() const noexcept
        {
        ...
        currently Microsoft SIGMA requires this to be the case for the parameters
        ...
        */
        ...
        using_ntt will be set to true. However, currently Microsoft SIGMA requires
    ...
    to store the encryption parameters, the SIGMAContext class is a heavy-weight class that
    ...
    for correctness and functionality by constructing an instance of SIGMAContext. The
    constructor of SIGMAContext does all of its work automatically, and concludes by
    ...
    and a new SIGMAContext will have to be created after the parameters are corrected.

    By default, SIGMAContext creates a chain of SIGMAContext::ContextData instances. The
    ...
    SIGMAContext. The functions key_context_data() and key_parms_id() return the ContextData
    ...
    */
    ...
    public:
        ...
        */
        ...
        public:
            ...
            */
            SIGMA_NODISCARD inline const EncryptionParameters &parms() const noexcept
            {
                return parms_;
            }
            ...
            */
            ...
            necessary to create a new instance of SIGMAContext once appropriate changes
            to the encryption parameters have been made.
            */
            SIGMA_NODISCARD inline EncryptionParameterQualifiers qualifiers() const noexcept
            {
                return qualifiers_;
            }
            ...
            */
            SIGMA_NODISCARD inline const std::uint64_t *total_coeff_modulus() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline int total_coeff_modulus_bit_count() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline const util::RNSTool *rns_tool() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline const util::NTTTables *small_ntt_tables() const noexcept
            {
            ...
            Returns a constant pointer to the NTT tables.
            */
            SIGMA_NODISCARD inline const util::NTTTables *plain_ntt_tables() const noexcept
            {
                return plain_ntt_tables_.get();
            }

            SIGMA_NODISCARD inline const util::NTTTables *device_plain_ntt_tables() const noexcept
            {
                return device_plain_ntt_tables_.get();
            }

            /**
            Returns a constant pointer to the GaloisTool.
            */
            SIGMA_NODISCARD inline const util::GaloisTool *galois_tool() const noexcept
            {
            ...
            plaintext modulus.
            */
            SIGMA_NODISCARD inline const util::MultiplyUIntModOperand *coeff_div_plain_modulus() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline std::uint64_t plain_upper_half_threshold() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline const std::uint64_t *plain_upper_half_increment() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline const std::uint64_t *upper_half_threshold() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline const std::uint64_t *upper_half_increment() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline std::uint64_t coeff_modulus_mod_plain_modulus() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline std::shared_ptr<const ContextData> prev_context_data() const noexcept
            {
            ...
            chain, then the result is nullptr.
            */
            SIGMA_NODISCARD inline std::shared_ptr<const ContextData> next_context_data() const noexcept
            {
            ...
            */
            SIGMA_NODISCARD inline std::size_t chain_index() const noexcept
            {
                return chain_index_;
            }
        ...
        private:
            ...
            util::DeviceArray<util::NTTTables> device_small_ntt_tables_;

            util::Pointer<util::NTTTables> plain_ntt_tables_;//预处理明文？
            util::DeviceArray<util::NTTTables> device_plain_ntt_tables_;
        ...
        /**
        Creates an instance of SIGMAContext and performs several pre-computations
        ...
        */
        SIGMAContext(
            ...
            : SIGMAContext(parms, expand_mod_chain, sec_level, MemoryManager::GetPool())
        {}

        /**
        Creates a new SIGMAContext by copying a given one.

        @param[in] copy The SIGMAContext to copy from
        */
        SIGMAContext(const SIGMAContext &copy) = default;

        /**
        Creates a new SIGMAContext by moving a given one.

        @param[in] source The SIGMAContext to move from
        */
        SIGMAContext(SIGMAContext &&source) = default;

        /**
        Copies a given SIGMAContext to the current one.

        @param[in] assign The SIGMAContext to copy from
        */
        SIGMAContext &operator=(const SIGMAContext &assign) = default;

        /**
        Moves a given SIGMAContext to the current one.

        @param[in] assign The SIGMAContext to move from
        */
        SIGMAContext &operator=(SIGMAContext &&assign) = default;
        ...
        */
        SIGMA_NODISCARD inline std::shared_ptr<const ContextData> get_context_data(parms_id_type parms_id) const
        {
        ...
        used for keys.
        */
        SIGMA_NODISCARD inline std::shared_ptr<const ContextData> key_context_data() const
        {
        ...
        */
        SIGMA_NODISCARD inline std::shared_ptr<const ContextData> first_context_data() const
        {
        ...
        that are used for data.
        */
        SIGMA_NODISCARD inline std::shared_ptr<const ContextData> last_context_data() const
        {
        ...
        */
        SIGMA_NODISCARD inline bool parameters_set() const
        {
        ...
        */
        SIGMA_NODISCARD inline const parms_id_type &key_parms_id() const noexcept
        {
        ...
        that are used for data.
        */
        SIGMA_NODISCARD inline const parms_id_type &first_parms_id() const noexcept
        {
        ...
        that are used for data.
        */
        SIGMA_NODISCARD inline const parms_id_type &last_parms_id() const noexcept
        {
        ...
        */
        SIGMA_NODISCARD inline bool using_keyswitching() const noexcept
        {
    ...
    private:
        ...
        */
        SIGMAContext(EncryptionParameters parms, bool expand_mod_chain, sec_level_type sec_level, MemoryPoolHandle pool);
