...
namespace sigma
{
    ...
    namespace util
    {
        namespace ztools
        {
            ...
            SIGMA_NODISCARD SizeT deflate_size_bound(SizeT in_size);
        } // namespace ztools
    } // namespace util

    /**
    A dynamic array for storing objects allocated from a Microsoft SIGMA memory
    ...
    */
    ...
    public:
        ...
        */
        SIGMA_NODISCARD inline T *begin() noexcept
        {
            return data_.get();
        }
        ...
        */
        SIGMA_NODISCARD inline const T *begin() const noexcept
        {
            return cbegin();
        }
        ...
        Returns a constant pointer to the beginning of the array data.
        */
        SIGMA_NODISCARD inline const T *cbegin() const noexcept
        {
            return data_.get();
        }
        ...
        */
        SIGMA_NODISCARD inline T *end() noexcept
        {
            return begin() + size_;
        }
        ...
        */
        SIGMA_NODISCARD inline const T *end() const noexcept
        {
            return cend();
        }
        ...
        Returns a constant pointer to the end of the array data.
        */
        SIGMA_NODISCARD inline const T *cend() const noexcept
        {
            return cbegin() + size_;
        }
#ifdef SIGMA_USE_MSGSL
        ...
        */
        SIGMA_NODISCARD inline gsl::span<T> span()
        {
        ...
        Returns a span pointing to the beginning of the DynArray.
        */
        SIGMA_NODISCARD inline gsl::span<const T> span() const
        {
        ...
        */
        SIGMA_NODISCARD inline const T &at(std::size_t index) const
        {
            if (index >= size_)
            {
        ...
        @throws std::out_of_range if index is out of range
        */
        SIGMA_NODISCARD inline T &at(std::size_t index)
        {
            if (index >= size_)
            {
                throw std::out_of_range("index must be within [0, size)");
        ...
        */
        SIGMA_NODISCARD inline bool empty() const noexcept
        {
            return (size_ == 0);
        }
        ...
        */
        SIGMA_NODISCARD inline std::size_t max_size() const noexcept
        {
        ...
        */
        SIGMA_NODISCARD inline std::size_t capacity() const noexcept
        {
            return capacity_;
        }
        ...
        */
        ...
        Microsoft SIGMA, if the loaded data is invalid, if decompression failed,
        ...
        */
        ...
        @throws std::runtime_error if I/O operations failed
        */
        inline std::streamoff load(const sigma_byte *in, std::size_t size, std::size_t in_size_bound = 0)
        {
            using namespace std::placeholders;
            return Serialization::Load(
    ...
    private:
        ...
        void load_members(std::istream &stream, SIGMA_MAYBE_UNUSED SIGMAVersion version, std::size_t in_size_bound)
        {
            auto old_except_mask = stream.exceptions();
            try
            {
