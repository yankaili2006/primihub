...
int PIRClient::generate_serialized_query(uint64_t desiredIndex,
  ...
  int N = enc_params_.poly_modulus_degree();//4096
  int output_size = 0;
  indices_ = compute_indices(desiredIndex, pir_params_.nvec);//可能多维
...
PirQuery PIRClient::generate_query(uint64_t desiredIndex) {
  ...
  for (uint32_t i = 0; i < indices_.size(); i++) {
    ...
    for (uint32_t j = 0; j < num_ptxts; j++) {
      ...
      Ciphertext dest;
      if (pir_params_.enable_symmetric) {
        encryptor_->encrypt_symmetric(pt, dest);//存储的非NTT形式
      } else {
...
std::vector<uint64_t> PIRClient::extract_coeffs(sigma::Plaintext pt,
                                                uint64_t offset) {
  vector<uint64_t> coeffs;
  encoder_->decode(pt, coeffs);
...
std::vector<uint8_t> PIRClient::extract_bytes(sigma::Plaintext pt,
                                              uint64_t offset) {
