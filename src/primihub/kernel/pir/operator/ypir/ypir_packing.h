...
// ---- Precomputed (fast online) packing -------------------------------------
//
// The per-query cost of PackManyLwes is the recursive butterfly inside
// PackLwes -> PackLwesInner: O(poly_len) homomorphic automorphisms (each a
// gadget-decompose + key-switch + NTTs). But that butterfly is a pure function
// of (prep_rlwe_cts, pub_params, y_constants) -- all FIXED across queries. The
// prepared cts carry only the db-hint a-vectors with a zero b-row (PrepPackLwes
// leaves row 1 zero); the per-query b_values enter only AFTER the butterfly,
// as an additive injection into the constant (row 1) polynomial. So PackLwes
// factors exactly as
//
//     PackLwes(b, prep, pub, y)
//       == inject_b( FromNtt(PackLwesInner(prep, pub, y)), b ).ToNtt()
//
// with PackLwesInner(prep, pub, y) loop-invariant. PrecomputePackManyLwes runs
// that invariant once (offline) and stores the raw butterfly result per output;
// PackManyLwesPrecomputed does only the cheap per-query suffix (copy + inject +
// NTT), turning the online pack from ~poly_len automorphs into one NTT.
//
// Equivalent to upstream precompute_pack / pack_using_precomp_vals, but our
// single-key OnExecute folds the key-switch (pub_params) into the offline
// butterfly -- upstream cannot, since its server reuses one precomputation
// across many clients and must re-apply each client's keys online.
//
// Result is bit-for-bit identical to PackManyLwes (see ypir_packing_test
// PackManyLwesPrecomputed_MatchesRecursive).

// Offline: per output, run PackLwesInner over the (query-independent) prepared
// cts and store the result in raw (coefficient) form. prep_rlwe_cts /
// num_rlwe_outputs / pub_params / y_constants as for PackManyLwes. Returns
// num_rlwe_outputs raw 2x1 matrices.
...
// Online: for each output, copy the precomputed raw result, inject this query's
// b_values into the constant row (coefficient z += b_values[i*poly_len+z] *
// poly_len, Barrett-reduced -- identical to PackLwes's out_raw loop), and NTT.
// No automorphs. b_values.size() == num_rlwe_outputs * poly_len; precomp from
// PrecomputePackManyLwes. Returns num_rlwe_outputs packed 2x1 NTT cts.
