...
class PSUHost(BaseModel):
    ...
    def _parse_params(self):
        ...
        # 是否广播结果给所有参与方
        self.broadcast_result = self.common_params.get("broadcast_result", True)
    ...
    def _create_psu(self, channel: Any):
        ...
        if self.protocol == "bloom":
            ...
            return PSUClass(role="host", channel=channel)

    def run(self):
        """执行隐私求并"""
        logger.info("PSUHost: Starting Private Set Union")
        ...
        channel = MultiGrpcClients(
            ...
            self.node_info,
            self.task_info,
        ) if len(guest_parties) > 1 else GrpcClient(
            self.node_info["self_party"],
            guest_parties[0],
            self.node_info,
            self.task_info,
        ...
        logger.info(f"PSUHost: Data loaded, shape={data.shape}")
        ...
        logger.info(f"PSUHost: Local IDs count = {len(local_ids)}")
        ...
        logger.info(f"PSUHost: Union computed, identifiable elements = {len(union_result)}")
        ...
        if self.output_mode == "data":
            ...
            if self.output_path:
                ...
                logger.info(f"PSUHost: Result data saved to {self.output_path}")
            ...
            logger.info(f"PSUHost: Union IDs saved to {self.output_path}")

        # 广播结果
        if self.broadcast_result:
            channel.send("union_complete", True)
            logger.info("PSUHost: Broadcast completion signal")

        logger.info("PSUHost: Completed")
        return result
