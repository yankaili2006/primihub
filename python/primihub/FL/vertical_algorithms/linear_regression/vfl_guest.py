"""
Vertical Federated Linear Regression - Guest
Guest方持有特征数据，参与联邦训练
"""
...
from .base import (
    LinearRegression_Guest_Plaintext,
    LinearRegression_Guest_CKKS
)


class VFLLinearRegressionGuest(BaseModel):
    """纵向联邦线性回归 - Guest方
...
class PlaintextGuest:
    ...
    def __init__(self, x, learning_rate, alpha, host_channel):
        self.model = LinearRegression_Guest_Plaintext(x, learning_rate, alpha)
...
class CKKSGuest(PlaintextGuest, CKKS):
    ...
    def __init__(self, x, learning_rate, alpha,
                 host_channel, coordinator_channel):
        self.t = 0
        self.model = LinearRegression_Guest_CKKS(x, learning_rate, alpha)
        self.host_channel = host_channel
