"""
Vertical Federated Logistic Regression - Host
...
class VFLLogisticRegressionHost(BaseModel):
    """纵向联邦逻辑回归 - Host方
    ...
    def train(self):
        ...
        if method == 'Plaintext':
            host = PlaintextHost(x, y, learning_rate, alpha, guest_channel)
        elif method == 'CKKS':
...
class PlaintextHost:
    ...
    def __init__(self, x, y, learning_rate, alpha, guest_channel):
        self.model = LogisticRegression_Host_Plaintext(
            x, y, learning_rate, alpha
        )
    ...
    def compute_final_metrics(self, x, y):
        ...
        if self.model.multiclass:
            ...
            metrics = classification_metrics(
                ...
                prefix="train_",
                metircs_name=[
                    "acc", "f1", "precision", "recall", "auc", "roc", "ks"
                ],
            )
        return metrics
...
class CKKSHost(PlaintextHost, CKKS):
    ...
    def __init__(self, x, y, learning_rate, alpha,
                 guest_channel, coordinator_channel):
        self.t = 0
        self.model = LogisticRegression_Host_CKKS(x, y, learning_rate, alpha)
        self.send_output_dim(guest_channel)
    ...
    def compute_enc_regular_loss(self):
        if self.model.alpha != 0:
            ...
            if self.model.multiclass:
                guest_regular_loss = [
                    self.load_tensor(s) for s in guest_regular_loss
                ]
            else:
                guest_regular_loss = [
                    self.load_vector(s) for s in guest_regular_loss
                ]
            return self.model.compute_regular_loss(sum(guest_regular_loss))
        else:
            return 0.
