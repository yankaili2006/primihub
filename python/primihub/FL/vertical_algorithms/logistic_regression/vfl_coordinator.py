"""
Vertical Federated Logistic Regression - Coordinator
Coordinator负责管理CKKS同态加密的密钥和模型解密
"""
...
class VFLLogisticRegressionCoordinator(BaseModel):
    """纵向联邦逻辑回归 - Coordinator
    ...
    def train(self):
        ...
        if method == 'CKKS':
            ...
            coordinator = CKKSCoordinator(
                batch_size, host_channel, guest_channel
            )
        else:
...
class CKKSCoordinator(CKKS):
    ...
    def recv_model(self):
        if self.multiclass:
            host_weight = self.load_tensor(
                self.host_channel.recv('host_weight')
            )
            host_bias = self.load_tensor(self.host_channel.recv('host_bias'))

            guest_weight = self.guest_channel.recv_all('guest_weight')
            guest_weight = [
                self.load_tensor(weight) for weight in guest_weight
            ]
        else:
            host_weight = self.load_vector(
                self.host_channel.recv('host_weight')
            )
            ...
            guest_weight = self.guest_channel.recv_all('guest_weight')
            guest_weight = [
                self.load_vector(weight) for weight in guest_weight
            ]
    ...
    def decrypt_model(self, host_weight, host_bias, guest_weight):
        ...
        host_weight = self.decrypt(host_weight, secret_key)
        host_bias = self.decrypt(host_bias, secret_key)

        guest_weight = [
            self.decrypt(weight, secret_key) for weight in guest_weight
        ]
    ...
    def encrypt_model(self, host_weight, host_bias, guest_weight):
        if self.multiclass:
            ...
            guest_weight = [
                self.encrypt_tensor(weight) for weight in guest_weight
            ]
        else:
            ...
            guest_weight = [
                self.encrypt_vector(weight) for weight in guest_weight
            ]
    ...
    def update_plaintext_model(self):
        ...
        if self.multiclass:
            ...
            guest_weight = [
                np.array(weight.tolist()).T for weight in guest_weight
            ]
        else:
    ...
    def compute_loss(self):
        ...
        if self.multiclass:
            ...
            loss = self.decrypt(
                loss, self.secret_context.secret_key()
            ).tolist()
        else:
