"""
Vertical Federated Logistic Regression - Guest
Guest方持有特征数据，参与联邦训练
"""
...
class VFLLogisticRegressionGuest(BaseModel):
    """纵向联邦逻辑回归 - Guest方
    ...
    def train(self):
        ...
        if method == 'Plaintext':
            guest = PlaintextGuest(x, learning_rate, alpha, host_channel)
        elif method == 'CKKS':
            guest = CKKSGuest(
...
class PlaintextGuest:
    ...
    def __init__(self, x, learning_rate, alpha, host_channel):
        ...
        self.model = LogisticRegression_Guest_Plaintext(
            x, learning_rate, alpha, output_dim
        )
...
class CKKSGuest(PlaintextGuest, CKKS):
    """CKKS同态加密训练的Guest实现"""
    ...
    def __init__(self, x, learning_rate, alpha,
        ...
        self.t = 0
        output_dim = self.recv_output_dim(host_channel)
        self.model = LogisticRegression_Guest_CKKS(
            x, learning_rate, alpha, output_dim
        )
    ...
    def update_ciphertext_model(self):
        self.coordinator_channel.send(
            'guest_weight', self.model.weight.serialize()
        )
    ...
    def send_enc_regular_loss(self):
        if self.model.alpha != 0.:
            guest_regular_loss = self.model.compute_regular_loss()
            self.host_channel.send(
                'guest_regular_loss', guest_regular_loss.serialize()
            )
