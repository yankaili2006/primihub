...
static void RunStatisticsFTest(std::string node_id, rpc::Task &task,
...
TEST(statistics_ftest, statistics_ftest_test) {
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_ftest_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    RunStatisticsFTest("node_1", task1, service);
    return;
  }
  pid = fork();
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    RunStatisticsFTest("node_2", task2, service);
    return;
  }
  sleep(3);
  primihub::Node node;
  ...
  registerDataSet(meta_infos, service);
  RunStatisticsFTest("node_3", task3, service);
}
