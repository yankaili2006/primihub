...
static void RunStatisticsMaxTest(std::string node_id, rpc::Task &task,
...
TEST(statistics_max, statistics_max_test) {
  ...
  std::string fucntion_name = "3";   //  "1为均值, 2为求和, 3为最大值, 4为最小值"
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_max_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    LOG(INFO) << "RunStatisticsMaxTest(node_1, task1, service);";
    RunStatisticsMaxTest("node_1", task1, service);
    return;
  }
  ...
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    LOG(INFO) << "RunStatisticsMaxTest(node_2, task2, service);";
    RunStatisticsMaxTest("node_2", task2, service);
    return;
  }
  ...
  registerDataSet(meta_infos, service);
  LOG(INFO) << "RunStatisticsMaxTest(node_3, task3, service);";
  RunStatisticsMaxTest("node_3", task3, service);
  return;
}
