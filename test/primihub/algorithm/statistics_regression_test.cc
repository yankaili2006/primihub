...
static void RunStatisticsRegression(std::string node_id, rpc::Task &task,
...
TEST(statistics_regression, statistics_regression_test) {
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_regression_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    RunStatisticsRegression("node_1", task1, service);
    return;
  }
  pid = fork();
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    RunStatisticsRegression("node_2", task2, service);
    return;
  }
  sleep(3);
  primihub::Node node;
  ...
  registerDataSet(meta_infos, service);
  RunStatisticsRegression("node_3", task3, service);
}
