...
static void RunStatisticsChiSquare(std::string node_id, rpc::Task &task,
...
TEST(statistics_chisquare, statistics_chisquare_test) {
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_chisquare_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    RunStatisticsChiSquare("node_1", task1, service);
    return;
  }
  pid = fork();
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    RunStatisticsChiSquare("node_2", task2, service);
    return;
  }
  sleep(3);
  primihub::Node node;
  ...
  registerDataSet(meta_infos, service);
  RunStatisticsChiSquare("node_3", task3, service);
}
