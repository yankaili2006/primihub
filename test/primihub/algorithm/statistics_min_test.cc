...
static void RunStatisticsMinTest(std::string node_id, rpc::Task &task,
...
TEST(statistics_min, statistics_min_test) {
  ...
  std::string fucntion_name = "4";   //  "1为均值, 2为求和, 3为最大值, 4为最小值"
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_min_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    LOG(INFO) << "RunStatisticsMinTest(node_1, task1, service);";
    RunStatisticsMinTest("node_1", task1, service);
    return;
  }
  ...
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    LOG(INFO) << "RunStatisticsMinTest(node_2, task2, service);";
    RunStatisticsMinTest("node_2", task2, service);
    return;
  }
  ...
  registerDataSet(meta_infos, service);
  LOG(INFO) << "RunStatisticsMinTest(node_3, task3, service);";
  RunStatisticsMinTest("node_3", task3, service);
  return;
}
