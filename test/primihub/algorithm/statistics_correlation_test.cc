...
static void RunStatisticsCorrelation(std::string node_id, rpc::Task &task,
...
TEST(statistics_correlation, statistics_correlation_test) {
  ...
  for (const auto& dataset_id : party_datasets) {
    ...
    out_file.append("mpc_correlation_party_").append(std::to_string(party_id)).append(".csv");
  ...
  if (pid != 0) {
    ...
    RunStatisticsCorrelation("node_1", task1, service);
    return;
  }
  pid = fork();
  if (pid != 0) {
    ...
    registerDataSet(meta_infos, service);
    RunStatisticsCorrelation("node_2", task2, service);
    return;
  }
  sleep(3);
  primihub::Node node;
  ...
  registerDataSet(meta_infos, service);
  RunStatisticsCorrelation("node_3", task3, service);
}
